...
type Step = 'perfil' | 'modalidades' | 'qualificacoes' | 'confirmar'
...
function safeNext(value: string | null) {
  if (!value || !value.startsWith('/') || value.startsWith('//')) return '/profissional/estado'
  return value
}
...
export default function ProfessionalRegisterPage() {
  ...
  const [qualifications, setQualifications] = useState<Array<{ title: string; issuer: string; issue_date: string }>>([])
  const [newQualification, setNewQualification] = useState({ title: '', issuer: '', issue_date: '' })
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (!user) {
        router.replace(`/auth/registar?type=professional&next=${encodeURIComponent(next)}`)
        return
      }
      const [{ data: existing }, { data: categoryRows }] = await Promise.all([
        supabase.from('professionals').select('id').eq('user_id', user.id).maybeSingle(),
        supabase.from('categories').select('*').order('name'),
      ])
      if (existing) {
        router.replace('/profissional/estado')
        return
      }
      setUserId(user.id)
  ...
  function toggleCategory(id: string) {
    setSelectedCategories(current => current.includes(id) ? current.filter(item => item !== id) : current.length < 5 ? [...current, id] : current)
  }
  ...
  async function submit() {
    ...
    setError(null)
    try {
      const result = await registerProfessionalInitial(formData.email, userId, {
        full_name: formData.full_name.trim(),
        professional_name: formData.professional_name.trim() || null,
        ...
        status: 'pending',
        gallery_urls: null,
      }, selectedCategories, qualifications)
      if (result.error) throw new Error(result.error)
      const supabase = createClient()
      await supabase.auth.updateUser({ data: { full_name: formData.full_name.trim(), type: 'professional' } })
      router.replace(next)
    } catch (err: any) {
      setError(err?.message || 'Não foi possível concluir o perfil profissional.')
    } finally {
      setSaving(false)
    }
  }
