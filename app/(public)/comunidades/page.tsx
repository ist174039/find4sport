...
type DiscoverCommunityRow = {
  item: DiscoverCommunity
...
export default async function CommunitiesPage({ searchParams }: { searchParams: Promise<{ category?: string; q?: string; sort?: string; page?: string }> }) {
  ...
  const search = (
    <form action="/comunidades" method="get" className="relative w-full max-w-3xl">
      ...
      <input name="q" defaultValue={filters.q || ''} placeholder="Pesquisar comunidades..." className="min-h-12 w-full rounded-xl border bg-background pl-10 pr-4 text-base outline-none focus:border-primary" />
              ...
              return (
                <Link key={community.id} href={`/comunidades/${community.slug || community.id}`} className="group flex min-w-0 flex-col overflow-hidden rounded-2xl border bg-card transition hover:border-primary/40 hover:shadow-md">
                  ...
                  <div className="flex flex-1 flex-col p-3">
                    <h2 className="line-clamp-1 text-sm font-bold group-hover:text-primary sm:text-base">{community.name}</h2>
                    <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted-foreground">{community.description || 'Comunidade desportiva na Find4Sport.'}</p>
                    {community.address && <p className="mt-2 flex items-center gap-1 text-[10px] text-muted-foreground"><MapPin className="h-3 w-3 shrink-0" /><span className="truncate">{community.address}</span></p>}
