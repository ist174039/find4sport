    ...
    <main className="min-h-screen bg-background pb-20 sm:pb-0">
      ...
      <EntityDetailLayout
        main={<>
          ...
          {gallery.length > 0 && <DetailSection title="Galeria" icon={<Images className="h-5 w-5 text-primary" />}><div className="grid grid-cols-2 gap-3 sm:grid-cols-3">{gallery.slice(0, 9).map((url: string, index: number) => <div key={url} className={`overflow-hidden rounded-xl bg-muted ${index === 0 ? 'col-span-2 aspect-[16/9] sm:col-span-2' : 'aspect-square'}`}><img src={url} alt={`${space.name} — fotografia ${index + 1}`} className="h-full w-full object-cover" /></div>)}</div></DetailSection>}
