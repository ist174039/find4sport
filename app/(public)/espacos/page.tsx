...
export default async function EspacosPage({searchParams}:PageProps){
  ...
  const categories=(categoryRows||[]) as Category[];const selected=filters.category?categories.find(c=>c.slug===filters.category||c.id===filters.category):undefined;const categoryIds=selected?branchIds(categories,selected.id):null
  const page=Math.max(1,Number.parseInt(filters.page||'1',10)||1);const sort=filters.sort||'relevance';const num=(value?:string)=>{const n=Number(value);return Number.isFinite(n)&&n>0?n:null}
  const{data,error:rpcError}=await (admin as any).rpc('discover_spaces',{p_lat:loc?.latitude??null,p_lng:loc?.longitude??null,p_radius:num(filters.radius),p_category_ids:categoryIds,p_q:filters.q||null,p_location:filters.location||null,p_rating:num(filters.rating),p_price_min:num(filters.priceMin),p_price_max:num(filters.priceMax),p_sort:sort,p_offset:(page-1)*PAGE_SIZE,p_limit:PAGE_SIZE});if(rpcError)throw new Error(`Não foi possível carregar espaços: ${rpcError.message}`)
  const rows=(data||[]) as Array<{item:any;total_count:number|string}>;const spaces=rows.map(r=>r.item) as (SportSpace&{categories:Category[];distanceKm?:number|null;averagePrice?:number|null})[];const total=Number(rows[0]?.total_count||0);const record=filters as Record<string,string|undefined>
