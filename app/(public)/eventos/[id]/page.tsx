...
export default async function EventProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const gallery = [...new Set([event.image_url, ...(Array.isArray(event.gallery_urls) ? event.gallery_urls : [])].filter(Boolean))] as string[]
  ...
  return <main className="min-h-screen bg-background pb-36 sm:pb-0">
    <EntityHero coverUrl={event.image_url} coverAlt={event.title || 'Evento'} avatar={<div className="flex h-20 w-20 flex-col items-center justify-center rounded-2xl border-2 border-white bg-background shadow-lg sm:h-24 sm:w-24">{startDate ? <><span className="text-2xl font-bold text-primary">{format(startDate, 'dd')}</span><span className="text-xs font-bold uppercase text-muted-foreground">{format(startDate, 'MMM', { locale: pt })}</span></> : <Calendar className="h-8 w-8 text-primary" />}</div>} title={event.title || 'Evento'} badges={<Badge className="bg-white/15 text-white hover:bg-white/20">Evento</Badge>} meta={<>{startDate && <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4" />{format(startDate, "d 'de' MMMM 'de' yyyy", { locale: pt })}</span>}{startDate && <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" />{format(startDate, 'HH:mm')}{endDate ? ` – ${format(endDate, 'HH:mm')}` : ''}</span>}{event.address && <span className="flex items-center gap-1.5"><MapPin className="h-4 w-4" />{event.address}</span>}</>} actions={actions} />
    ...
    <EntityDetailLayout main={<>
      ...
      <DetailSection title="Informação do evento" icon={<Calendar className="h-5 w-5 text-primary" />}><div className="grid gap-4 sm:grid-cols-2"><DetailStat label="Preço" value={formatPrice(event.price_min, event.price_max)} /><DetailStat label="Capacidade" value={event.capacity ? `${event.capacity} participantes` : 'Não indicada'} /><DetailStat label="Data" value={startDate ? format(startDate, "d 'de' MMMM 'de' yyyy", { locale: pt }) : 'Não indicada'} /><DetailStat label="Horário" value={startDate ? `${format(startDate, 'HH:mm')}${endDate ? ` – ${format(endDate, 'HH:mm')}` : ''}` : 'Não indicado'} /></div></DetailSection>
      {gallery.length > 1 && <DetailSection title="Galeria"><EntityGallery images={gallery} alt={event.title || 'Evento'} /></DetailSection>}
    </>} aside={<>
      <DetailSection title="Inscrição" icon={<Ticket className="h-5 w-5 text-primary" />}><p className="mb-4 text-2xl font-bold">{formatPrice(event.price_min, event.price_max)}</p><JoinEventBtn eventId={event.id} eventPrice={event.price_min || 0} /></DetailSection>
