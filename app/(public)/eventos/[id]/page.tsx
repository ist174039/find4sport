...
function ticketSummary(tickets: TicketType[]) {
  if (!tickets.length) return 'Grátis'
  const prices = tickets.map(ticket => Number(ticket.price)).filter(Number.isFinite)
  if (!prices.length || Math.max(...prices) === 0) return 'Grátis'
...
export default async function EventProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const finished = Boolean(startDate && startDate.getTime() <= Date.now())
  ...
  const actions = finished
    ...
    : <JoinEventBtn eventId={event.id} eventPrice={Number(activeTickets[0]?.price || 0)} ticketTypes={ticketTypes} />
  ...
  return (
    <main className="min-h-screen bg-background pb-36 sm:pb-0">
      <EntityHero
        ...
        badges={<><Badge className="bg-white/15 text-white">Evento</Badge>{finished && <Badge variant="secondary">Terminado</Badge>}</>}
        meta={<>{startDate && <span className="flex items-center gap-1.5"><Calendar className="h-4 w-4" />{format(startDate, "d 'de' MMMM 'de' yyyy", { locale: pt })}</span>}{startDate && <span className="flex items-center gap-1.5"><Clock className="h-4 w-4" />{format(startDate, 'HH:mm')}{endDate ? ` – ${format(endDate, 'HH:mm')}` : ''}</span>}{event.address && <span className="flex items-center gap-1.5"><MapPin className="h-4 w-4" />{event.address}</span>}<span className="font-semibold">{priceLabel}</span></>}
        actions={actions}
      />

      <EntityDetailLayout
        main={<>
          <DetailSection title="Sobre o evento">{event.description ? <p className="whitespace-pre-line text-sm leading-relaxed text-muted-foreground sm:text-base">{event.description}</p> : <p className="text-sm text-muted-foreground">O organizador ainda não adicionou uma descrição.</p>}</DetailSection>
          <DetailSection title="Informação do evento" icon={<Calendar className="h-5 w-5 text-primary" />}><div className="grid gap-4 sm:grid-cols-2"><DetailStat label="Preço" value={priceLabel} /><DetailStat label="Participantes" value={event.capacity ? `${participantCount || 0} / ${event.capacity}` : `${participantCount || 0} confirmados`} /><DetailStat label="Lugares disponíveis" value={remaining == null ? 'Sem limite indicado' : remaining} /><DetailStat label="Data" value={startDate ? format(startDate, "d 'de' MMMM 'de' yyyy", { locale: pt }) : 'Não indicada'} /><DetailStat label="Horário" value={startDate ? `${format(startDate, 'HH:mm')}${endDate ? ` – ${format(endDate, 'HH:mm')}` : ''}` : 'Não indicado'} /></div></DetailSection>
          {activeTickets.length > 0 && <DetailSection title="Bilhetes" icon={<Ticket className="h-5 w-5 text-primary" />}><div className="grid gap-3 sm:grid-cols-2">{activeTickets.map(ticket => <div key={ticket.id} className="rounded-xl border p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{ticket.name}</p>{ticket.description && <p className="mt-1 text-xs text-muted-foreground">{ticket.description}</p>}</div><p className="shrink-0 font-bold">{Number(ticket.price) === 0 ? 'Grátis' : `${Number(ticket.price).toFixed(2)} €`}</p></div>{ticket.capacity && <p className="mt-2 text-xs text-muted-foreground">Lotação deste bilhete: {ticket.capacity}</p>}</div>)}</div></DetailSection>}
          {gallery.length > 1 && <DetailSection title="Galeria"><EntityGallery images={gallery} alt={event.title || 'Evento'} /></DetailSection>}
        </>}
        aside={<>
          <DetailSection title={finished ? 'Estado' : 'Inscrição'} icon={<Ticket className="h-5 w-5 text-primary" />}><p className="mb-4 text-2xl font-bold">{priceLabel}</p>{actions}</DetailSection>
          {event.address && <DetailSection title="Localização" icon={<MapPin className="h-5 w-5 text-primary" />}><p className="mb-3 text-sm leading-relaxed text-muted-foreground">{event.address}</p><ObterDirecoesBtn address={event.address} name={event.title} latitude={event.latitude} longitude={event.longitude} /></DetailSection>}
          {organizerName && <DetailSection title="Organização" icon={<UserRound className="h-5 w-5 text-primary" />}>{profileHref ? <Link href={profileHref} className="flex min-h-14 items-center gap-3 rounded-xl border p-3"><div className="relative flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-primary/10 font-bold text-primary">{professional?.avatar_url ? <AppImage src={professional.avatar_url} alt={organizerName} fill sizes="44px" className="object-cover" /> : organizerName.charAt(0)}</div><div className="min-w-0"><p className="truncate text-sm font-semibold">{organizerName}</p><p className="text-xs text-primary">Ver perfil</p></div></Link> : <div className="flex items-center gap-3"><Users className="h-5 w-5 text-primary" /><p className="text-sm font-semibold">{organizerName}</p></div>}</DetailSection>}
        </>}
      />
