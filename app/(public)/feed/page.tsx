...
export default async function Page({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  if (tabParam === 'following') {
    if (!user || followingIds.length === 0) postsQuery = postsQuery.eq('id', '00000000-0000-0000-0000-000000000000')
    else {
  ...
  ;(recentContent || []).forEach((post: any) => { const matches = post.content?.match(/#[\wÀ-ÿ]+/g) || []; matches.forEach((tag: string) => { tagCounts[tag] = (tagCounts[tag] || 0) + 1 }) })
  ...
  return <div className="min-h-screen bg-muted/15 py-4 sm:py-8"><PageContainer><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]">
    <main className="min-w-0 space-y-4 sm:space-y-5"><div className="sticky top-16 z-20 -mx-4 border-b border-border bg-background/95 px-4 py-2 backdrop-blur sm:static sm:mx-0 sm:rounded-2xl sm:border sm:p-2 sm:shadow-sm"><div className="flex items-center gap-2"><Link href="/feed?tab=foryou" className={`flex min-h-11 flex-1 items-center justify-center rounded-xl px-4 text-sm font-semibold transition sm:flex-none ${tabParam !== 'following' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`}>Para ti</Link>{user && <Link href="/feed?tab=following" className={`flex min-h-11 flex-1 items-center justify-center rounded-xl px-4 text-sm font-semibold transition sm:flex-none ${tabParam === 'following' ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted'}`}>A seguir</Link>}<div className="ml-auto"><FeedFilterModal /></div></div></div>
