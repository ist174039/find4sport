...
export default async function Page({ searchParams }: { searchParams: Promise<{ [key: string]: string | string[] | undefined }> }) {
  ...
  let currentUserType = 'user', currentUserName = '', currentUserAvatar = ''; let canPublish = false
  if (user) {
    const { data: profile } = await supabase.from('platform_users').select('type,full_name,avatar_url').eq('id', user.id).single()
    if (profile) {
      const normalizedRole = normalizePlatformRole(profile.type); currentUserType = profile.type || 'user'; currentUserName = profile.full_name || ''; currentUserAvatar = profile.avatar_url || ''
  ...
  if(user){const{data:follows}=await supabase.from('user_follows').select('following_id').eq('follower_id',user.id);followingIds=(follows||[]).map(f=>f.following_id)}
  let followedProfRows:any[]=[], followedSpaceRows:any[]=[]
  if(followingIds.length){const[{data:profs},{data:spaces}]=await Promise.all([supabase.from('professionals').select('id,user_id,full_name,avatar_url,public_slug,is_verified,rating_avg,review_count').in('user_id',followingIds).eq('is_verified',true).limit(20),supabase.from('sport_spaces').select('id,owner_user_id,name,logo_url,slug,is_verified,rating_avg,review_count').in('owner_user_id',followingIds).eq('is_verified',true).limit(20)]);followedProfRows=profs||[];followedSpaceRows=spaces||[]}
  ...
  if(tabParam==='following'){if(!user||!followingIds.length)postsQuery=postsQuery.eq('id','00000000-0000-0000-0000-000000000000');else{const profIds=followedProfRows.map(p=>p.id),spaceIds=followedSpaceRows.map(s=>s.id),clauses:string[]=[];if(profIds.length)clauses.push(`professional_id.in.(${profIds.join(',')})`);if(spaceIds.length)clauses.push(`sport_space_id.in.(${spaceIds.join(',')})`);postsQuery=clauses.length?postsQuery.or(clauses.join(',')):postsQuery.eq('id','00000000-0000-0000-0000-000000000000')}}
  if(authorTypeParam==='pro')postsQuery=postsQuery.not('professional_id','is',null);if(authorTypeParam==='space')postsQuery=postsQuery.not('sport_space_id','is',null)
  if(dateParam==='today'){const date=new Date();date.setHours(0,0,0,0);postsQuery=postsQuery.gte('created_at',date.toISOString())}else if(dateParam==='week')postsQuery=postsQuery.gte('created_at',new Date(Date.now()-7*86400000).toISOString());else if(dateParam==='month')postsQuery=postsQuery.gte('created_at',new Date(Date.now()-30*86400000).toISOString())
  const keyword=categoryParam||searchParam;if(keyword)postsQuery=postsQuery.ilike('content',`%${keyword}%`);const{data:posts}=await postsQuery
  let likedPostIds=new Set<string>();if(user&&posts?.length){const{data:likes}=await supabase.from('post_likes').select('post_id').eq('user_id',user.id).in('post_id',posts.map((p:any)=>p.id));likedPostIds=new Set((likes||[]).map((like:any)=>like.post_id))}

  const {data:recentContent}=await supabase.from('posts').select('content').is('community_id',null).order('created_at',{ascending:false}).limit(250);const tagCounts:Record<string,number>={};(recentContent||[]).forEach((post:any)=>{const matches=post.content?.match(/#[\wÀ-ÿ]+/g)||[];matches.forEach((tag:string)=>{tagCounts[tag]=(tagCounts[tag]||0)+1})});const trendingTags=Object.entries(tagCounts).map(([tag,count])=>({tag,count})).sort((a,b)=>b.count-a.count).slice(0,5)
  const[{data:suggestedProfsRaw},{data:suggestedSpacesRaw}]=await Promise.all([supabase.from('professionals').select('id,user_id,full_name,avatar_url,public_slug,is_verified,rating_avg,review_count').eq('is_verified',true).eq('status','active').limit(20),supabase.from('sport_spaces').select('id,owner_user_id,name,logo_url,slug,is_verified,rating_avg,review_count').eq('is_verified',true).limit(20)])
  const followedUserIds=new Set(followingIds);const score=(x:any)=>Number(x.review_count||0)*2+Number(x.rating_avg||0);const suggestedProfs=(suggestedProfsRaw||[]).filter((p:any)=>p.user_id!==user?.id&&!followedUserIds.has(p.user_id)).sort((a:any,b:any)=>score(b)-score(a)).slice(0,3);const suggestedSpaces=(suggestedSpacesRaw||[]).filter((s:any)=>s.owner_user_id&&s.owner_user_id!==user?.id&&!followedUserIds.has(s.owner_user_id)).sort((a:any,b:any)=>score(b)-score(a)).slice(0,2)
