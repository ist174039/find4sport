  ...
  return (
    <main className="min-h-screen bg-background pb-20">
      ...
      <section className="mx-auto max-w-7xl space-y-10 px-4 py-8 sm:px-6 sm:py-10 lg:px-8">
        {safeCategories.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-10 text-center"><Activity className="mx-auto h-9 w-9 text-muted-foreground/40" /><h2 className="mt-3 font-semibold">Nenhuma modalidade encontrada</h2><p className="mt-1 text-sm text-muted-foreground">Tenta outra pesquisa ou sugere uma nova modalidade.</p></div> : groups.map(group => <section key={group.id} id={group.id} className="scroll-mt-24"><div className="mb-4 flex items-start gap-3"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-primary/10 text-2xl">{group.emoji}</div><div><h2 className="text-xl font-bold text-foreground sm:text-2xl">{group.name}</h2><p className="mt-1 text-sm text-muted-foreground">{group.description}</p></div></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{group.sports.map((cat: any) => <article key={cat.id} className="group overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-md"><Link href={`/pesquisa?category=${encodeURIComponent(cat.slug || cat.name)}`} className="flex min-h-24 items-center gap-4 p-4"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-muted text-3xl transition group-hover:bg-primary/10">{cat.emoji || group.emoji}</div><div className="min-w-0 flex-1"><h3 className="truncate font-semibold text-foreground group-hover:text-primary">{cat.name}</h3><p className="mt-1 text-xs text-muted-foreground">Ver tudo nesta modalidade</p></div><ArrowRight className="h-4 w-4 shrink-0 text-muted-foreground transition group-hover:translate-x-0.5 group-hover:text-primary" /></Link><div className="grid grid-cols-3 border-t border-border bg-muted/15 text-[11px] font-semibold"><Link href={`/profissionais?category=${encodeURIComponent(cat.slug || cat.name)}`} className="min-h-10 px-2 py-3 text-center hover:bg-muted hover:text-primary">Profissionais</Link><Link href={`/espacos?category=${encodeURIComponent(cat.slug || cat.name)}`} className="min-h-10 border-x border-border px-2 py-3 text-center hover:bg-muted hover:text-primary">Espaços</Link><Link href={`/eventos?category=${encodeURIComponent(cat.slug || cat.name)}`} className="min-h-10 px-2 py-3 text-center hover:bg-muted hover:text-primary">Eventos</Link></div></article>)}</div></section>)}
