...
export default async function Page(){
  ...
  const [spacesCount,profsCount,eventsCount,spacesResult,prosResult,eventsResult,carouselRes]=await Promise.all([
    ...
    admin.from('sport_spaces').select('id,name,slug,address,rating_avg,review_count,gallery_urls,cover_url,is_verified,latitude,longitude,status').or('status.eq.active,is_verified.eq.true').limit(80),
    admin.from('professionals').select('id,user_id,full_name,professional_name,public_slug,address,rating_avg,review_count,avatar_url,is_verified,status,views_count,latitude,longitude').eq('status','active').limit(120),
    admin.from('events').select('id,title,start_date,address,image_url,status,latitude,longitude,price_min').eq('status','published').gte('start_date',now).order('start_date',{ascending:true}).limit(80),
    admin.from('carousel_slides').select('*').eq('is_active',true).order('display_order',{ascending:true}),
  ])
  const candidates=prosResult.data||[];const proIds=candidates.map(p=>p.id);const userIds=candidates.map(p=>p.user_id);const spaceIds=(spacesResult.data||[]).map(s=>s.id)
  const [follows,completed,services,posts,subs,roomPrices]=await Promise.all([
    userIds.length?admin.from('user_follows').select('following_id').in('following_id',userIds):Promise.resolve({data:[] as any[]}),
    proIds.length?admin.from('reservations').select('professional_id').in('professional_id',proIds).eq('status','completed'):Promise.resolve({data:[] as any[]}),
    ...
    proIds.length?admin.from('posts').select('id,professional_id').in('professional_id',proIds):Promise.resolve({data:[] as any[]}),
    userIds.length?admin.from('user_subscriptions').select('user_id,tier,status').in('user_id',userIds):Promise.resolve({data:[] as any[]}),
  ...
  const followers=new Map<string,number>(),done=new Map<string,number>(),activeServices=new Map<string,number>(),commentScore=new Map<string,number>(),servicePrices=new Map<string,number[]>(),spacePrices=new Map<string,number[]>()
  for(const r of follows.data||[])bump(followers,r.following_id);for(const r of completed.data||[])bump(done,r.professional_id);for(const r of services.data||[]){bump(activeServices,r.professional_id);const price=Number(r.price);if(Number.isFinite(price)&&price>0)servicePrices.set(r.professional_id,[...(servicePrices.get(r.professional_id)||[]),price])}
  for(const r of roomPrices.data||[]){const price=Number(r.price_per_hour);if(Number.isFinite(price)&&price>0)spacePrices.set(r.space_id,[...(spacePrices.get(r.space_id)||[]),price])}
  ...
  const professionals=candidates.map((p:any)=>{const d=distanceFrom(userLocation,p.latitude,p.longitude);const plan=String(plans.get(p.user_id)||'free');const score=Number(p.rating_avg||0)*10+Math.min(Number(p.review_count||0),20)*1.5+Math.min(followers.get(p.user_id)||0,50)*.5+Math.log10(Number(p.views_count||0)+1)*4+Math.min(done.get(p.id)||0,20)*2+Math.min(commentScore.get(p.id)||0,20)*.5+Math.min(activeServices.get(p.id)||0,10)+(p.is_verified?3:0)+(plan==='premium'?5:plan==='pro'?2:0)+nearBonus(d);return{...p,distanceKm:d,score,averagePrice:average(servicePrices.get(p.id)||[])}}).sort((a,b)=>b.score-a.score).slice(0,6)
  const spaces=(spacesResult.data||[]).map((s:any)=>({...s,distanceKm:distanceFrom(userLocation,s.latitude,s.longitude),averagePrice:average(spacePrices.get(s.id)||[])})).sort((a,b)=>{if(userLocation){const d=(a.distanceKm??Infinity)-(b.distanceKm??Infinity);if(d)return d}return Number(b.rating_avg||0)-Number(a.rating_avg||0)}).slice(0,6)
  const events=(eventsResult.data||[]).map((e:any)=>({...e,distanceKm:distanceFrom(userLocation,e.latitude,e.longitude)})).sort((a,b)=>{if(userLocation){const d=(a.distanceKm??Infinity)-(b.distanceKm??Infinity);if(d)return d}return new Date(a.start_date).getTime()-new Date(b.start_date).getTime()}).slice(0,6)
