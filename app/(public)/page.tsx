...
function ImageOrFallback({ src, alt }: { src?: string | null; alt: string }) {
  return <div className="relative aspect-square overflow-hidden bg-muted"><AppImage src={src} alt={alt} fill sizes="(max-width: 640px) 50vw, (max-width: 1280px) 33vw, 16vw" className="object-cover" /></div>
}
...
export default async function Page() {
  ...
  const [spacesCount, profsCount, eventsCount, spacesResult, prosResult, eventsResult, carouselRes] = await Promise.all([
    ...
    admin.from('events').select('id', { count: 'exact', head: true }).eq('status', PUBLIC_EVENT_STATUS),
