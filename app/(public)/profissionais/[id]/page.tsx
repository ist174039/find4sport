...
export default async function ProfessionalProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: catData }, { data: qualifications }, { data: memberData }, { data: services }, { data: associatedSpace }, { count: followersCount }, { data: { user } }] = await Promise.all([
    ...
    supabase.from('sport_spaces').select('*').or(`owner_user_id.eq.${professional.user_id},created_by.eq.${professional.user_id}`).maybeSingle(),
  ...
  const followAction = user && user.id !== professional.user_id
    ? <FollowButton targetUserId={professional.user_id} initialIsFollowing={isFollowing} className="min-h-11 rounded-xl px-4" />
    : null
  ...
  return (
    <main className="min-h-screen bg-background pb-20 sm:pb-0">
      <EntityHero
        coverUrl={coverUrl}
        coverAlt={`Capa de ${displayName}`}
        avatar={<div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-2xl border-2 border-white bg-background text-2xl font-bold text-primary shadow-lg sm:h-24 sm:w-24">{avatarUrl ? <img src={avatarUrl} alt={displayName} className="h-full w-full object-cover" /> : displayName?.charAt(0)?.toUpperCase()}</div>}
        title={displayName}
        subtitle={professional.professional_name && professional.full_name !== professional.professional_name ? professional.full_name : undefined}
        ...
        meta={<>{professional.address && <span className="flex items-center gap-1"><MapPin className="h-4 w-4" />{professional.address}</span>}{professional.rating_avg > 0 && <span className="flex items-center gap-1 text-amber-300"><Star className="h-4 w-4 fill-current" />{Number(professional.rating_avg).toFixed(1)} ({professional.review_count || 0})</span>}<span className="flex items-center gap-1"><Users className="h-4 w-4" />{followersCount || 0} seguidores</span></>}
        actions={<>{followAction}{contactAction}</>}
      />
      ...
      <EntityDetailLayout
        main={<>
          ...
          <DetailSection title="Serviços" icon={<Dumbbell className="h-5 w-5 text-primary" />}>
          ...
          {qualifications && qualifications.length > 0 && (
            <DetailSection title="Qualificações" icon={<Award className="h-5 w-5 text-amber-500" />}>
              <div className="grid gap-3 sm:grid-cols-2">{qualifications.map((qualification: any) => <div key={qualification.id} className="rounded-xl border border-border bg-muted/20 p-4"><p className="font-semibold text-foreground">{qualification.title}</p>{qualification.issuer && <p className="mt-1 text-sm text-muted-foreground">{qualification.issuer}</p>}</div>)}</div>
            </DetailSection>
          )}
        ...
        aside={<>
          <DetailSection title="Resumo">
            <div className="grid grid-cols-2 gap-4 lg:grid-cols-1">
              <DetailStat label="Avaliação" value={professional.rating_avg > 0 ? `${Number(professional.rating_avg).toFixed(1)} / 5` : 'Sem avaliações'} />
              <DetailStat label="Seguidores" value={followersCount || 0} />
              <DetailStat label="Serviços ativos" value={(services || []).length} />
              {professional.address && <DetailStat label="Localização" value={professional.address} />}
            </div>
            {professional.address && <div className="mt-4"><ObterDirecoesBtn address={professional.address} name={displayName} latitude={professional.latitude} longitude={professional.longitude} /></div>}
          </DetailSection>

          {associatedSpace && <DetailSection title="Espaço associado" icon={<Building2 className="h-5 w-5 text-primary" />}><Link href={`/espacos/${associatedSpace.slug || associatedSpace.id}`} className="block rounded-xl border border-border p-3 transition hover:border-primary/40"><p className="font-semibold text-foreground">{associatedSpace.name}</p><p className="mt-1 text-sm text-muted-foreground">{associatedSpace.address || 'Ver espaço'}</p></Link></DetailSection>}
