...
async function getProfessionalsData(searchParams: { category?: string; q?: string; location?: string; sort?: string }) {
  ...
  if (searchParams.location) query = query.or(`address.ilike.%${searchParams.location}%,location.ilike.%${searchParams.location}%`)
  ...
  const { data: professionalRows, error: professionalsError } = await query.limit(24)
  ...
  const transformed = professionals.map(pro => ({ ...pro, categories: categoryMap.get(pro.id) || [] }))
  ...
  return { categories, professionals: transformed as (Professional & { categories: Category[] })[], filters: searchParams }
}
