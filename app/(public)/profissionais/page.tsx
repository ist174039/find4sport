...
async function getProfessionalsData(filters: { category?: string; q?: string; location?: string; sort?: string; radius?: string; rating?: string; priceMin?: string; priceMax?: string }) {
  ...
  const { data: rows, error } = await query.limit(150)
  ...
  if (professionals.length) {
    ...
    for (const link of links || []) { const category=byId.get(link.category_id); if(!category)continue; categoryMap.set(link.professional_id,[...(categoryMap.get(link.professional_id)||[]),category]) }
  }

  let transformed = professionals.map(pro => ({ ...pro, categories: categoryMap.get(pro.id) || [], distanceKm: distanceFrom(userLocation, pro.latitude, pro.longitude) }))
  const radius = Number(filters.radius)
  if (userLocation && Number.isFinite(radius) && radius > 0) transformed = transformed.filter(item => item.distanceKm != null && item.distanceKm <= radius)
  const sortBy = filters.sort || 'relevance'
  transformed.sort((a,b)=>{ if(sortBy==='rating')return Number(b.rating_avg||0)-Number(a.rating_avg||0); if(sortBy==='reviews')return Number(b.review_count||0)-Number(a.review_count||0); if(sortBy==='newest')return new Date(b.created_at).getTime()-new Date(a.created_at).getTime(); if(userLocation)return(a.distanceKm??Infinity)-(b.distanceKm??Infinity); return Number(b.rating_avg||0)-Number(a.rating_avg||0) })
  return { categories, professionals: transformed as (Professional & { categories: Category[]; distanceKm?: number | null })[], filters }
}

function href(base:string, filters:Record<string,string|undefined>, updates:Record<string,string|undefined|null>){const params=new URLSearchParams();Object.entries({...filters,...updates}).forEach(([key,value])=>{if(!value||(key==='sort'&&value==='relevance'))return;params.set(key,value)});return params.size?`${base}?${params}`:base}
...
export default async function ProfissionaisPage({ searchParams }: PageProps) {
  ...
  return <DiscoveryPage title={selectedCategory ? `Profissionais de ${selectedCategory.name}` : 'Profissionais de Desporto'} description="Encontra profissionais ativos por modalidade, proximidade, localização e reputação." countLabel={`${professionals.length} ${professionals.length===1?'profissional encontrado':'profissionais encontrados'}`} search={<div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_320px]"><SearchBar defaultQuery={filters.q} defaultLocation={filters.location} defaultType="profissionais" showType={false} basePath="/profissionais" showFilters filterType="profissionais" currentFilters={filters as Record<string,string>} placeholder="Pesquisar profissionais…"/><DiscoveryTaxonomyFilter basePath="/profissionais" categories={categories} currentCategory={filters.category} query={filters.q} location={filters.location} sort={filters.sort}/></div>} sorts={[{label:'Mais próximos',href:href('/profissionais',filterRecord,{sort:'relevance'}),active:currentSort==='relevance'},{label:'Melhor avaliados',href:href('/profissionais',filterRecord,{sort:'rating'}),active:currentSort==='rating'},{label:'Mais avaliados',href:href('/profissionais',filterRecord,{sort:'reviews'}),active:currentSort==='reviews'},{label:'Mais recentes',href:href('/profissionais',filterRecord,{sort:'newest'}),active:currentSort==='newest'}]} clearHref={filters.q||filters.location||filters.category||filters.radius||currentSort!=='relevance'?'/profissionais':undefined}>
    {professionals.length?<ProfessionalGrid professionals={professionals} columns={3}/>:<DiscoveryEmptyState title="Nenhum profissional encontrado" description="Não existem profissionais ativos para esta combinação de filtros ou raio." clearHref="/profissionais"/>}
  </DiscoveryPage>
}
