...
export async function reviewCommunityJoinRequestAction(requestId: string, decision: 'approve' | 'reject'): Promise<void> {
  const supabase = await createClientServer()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Acesso negado. Sessão não encontrada.')
  const admin = createAdminClient()
  ...
  revalidatePath(`/dashboard/comunidades/${request.community_id}`)
}
