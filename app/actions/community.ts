...
export async function joinCommunityAction(communityId: string) {
  ...
  if (community.is_private) {
    ...
    if (existingRequest?.status === 'pending') return { success: true, status: 'pending' as const, message: 'O pedido de adesão já está pendente.' }
...
export async function leaveCommunityAction(communityId: string) {
  ...
  if (membership.role === 'admin') {
    const { count } = await admin
      ...
      .eq('role', 'admin')
    if ((count || 0) <= 1) {
      throw new Error('Antes de sair, atribua a administração da comunidade a outro membro.')
    }
  }
...
export async function reviewCommunityJoinRequestAction(requestId: string, decision: 'approve' | 'reject') {
  ...
  const { error } = await admin.from('community_join_requests').update({
...
export async function createCommunityAction(formData: { name: string; description: string; privacy: string; city: string; modality: string }) {
  ...
  try {
    ...
    throw new Error(error?.message || 'O seu plano atual não permite criar comunidades.')
  }
  ...
  if (limit !== null) {
    ...
    if (countError) throw new Error('Não foi possível validar o limite de comunidades do seu plano.')
  ...
  // These fields exist in some deployments but not in older schemas. Creation
  // must not fail merely because an older deployment does not have them yet.
  const optionalPayload = {
    sport_category: String(formData.modality || '').trim() || null,
    location: String(formData.city || '').trim() || null,
  }

  let newCommunity: any = null
  let error: any = null
  ;({ data: newCommunity, error } = await admin.from('communities').insert({ ...insertPayload, ...optionalPayload }).select('id').single())
  if (error && (error.code === '42703' || /column .* does not exist/i.test(error.message || ''))) {
    ;({ data: newCommunity, error } = await admin.from('communities').insert(insertPayload).select('id').single())
  }
