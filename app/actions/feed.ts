...
export async function createPostAction(formData: FormData) {
  ...
  try {
    ...
    if (file) {
      ...
      uploadedPath = `${user.id}/posts/${crypto.randomUUID()}.${extension}`
      ...
      const { data: publicUrlData } = admin.storage.from('avatars').getPublicUrl(uploadedPath)
    ...
    if (uploadedPath) {
      try { createAdminClient().storage.from('avatars').remove([uploadedPath]) } catch { /* best effort cleanup */ }
    }
