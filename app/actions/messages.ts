...
async function resolveMessagingContext(userA: string, userB: string): Promise<MessagingContext> {
  ...
  const resolvePair = async (athleteId: string, providerId: string): Promise<MessagingContext> => {
...
async function ensureThread(context: MessagingContext) {
  if (!context.allowed || !context.contextId || !context.contextType || !context.athleteId || !context.providerUserId) return null
