...
export default function AdminSettingsPage() {
  ...
  if (loading) {
    return <DashboardPage><DashboardPageHeader title="Definições" description="Apenas configurações que têm efeito real no runtime da plataforma são apresentadas aqui." /><DashboardLoadingState label="A carregar definições…" /></DashboardPage>
  }
  ...
  return (
    <DashboardPage>
      ...
      <DashboardSection
        ...
        action={<Dialog open={dialogOpen} onOpenChange={setDialogOpen}><DialogTrigger render={<Button className="min-h-11"><Plus className="mr-2 h-4 w-4" />Adicionar slide</Button>} /><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg"><DialogHeader><DialogTitle>Novo slide</DialogTitle></DialogHeader><div className="grid gap-4 pt-2"><div className="space-y-2"><Label htmlFor="carousel-image-url">URL da imagem *</Label><Input id="carousel-image-url" required className="min-h-11 text-base" value={form.imageUrl} onChange={e => setForm(v => ({ ...v, imageUrl: e.target.value }))} /></div><div className="space-y-2"><Label htmlFor="carousel-title">Título</Label><Input id="carousel-title" className="min-h-11 text-base" value={form.title} onChange={e => setForm(v => ({ ...v, title: e.target.value }))} /></div><div className="space-y-2"><Label htmlFor="carousel-subtitle">Subtítulo</Label><Textarea id="carousel-subtitle" className="min-h-24 text-base" value={form.subtitle} onChange={e => setForm(v => ({ ...v, subtitle: e.target.value }))} /></div><div className="grid gap-4 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor="carousel-button-text">Texto do botão</Label><Input id="carousel-button-text" className="min-h-11 text-base" value={form.buttonText} onChange={e => setForm(v => ({ ...v, buttonText: e.target.value }))} /></div><div className="space-y-2"><Label htmlFor="carousel-button-link">Link do botão</Label><Input id="carousel-button-link" className="min-h-11 text-base" value={form.buttonLink} onChange={e => setForm(v => ({ ...v, buttonLink: e.target.value }))} placeholder="/profissionais" /></div></div><div className="space-y-2"><Label htmlFor="carousel-order">Ordem</Label><Input id="carousel-order" type="number" className="min-h-11 text-base" value={form.displayOrder} onChange={e => setForm(v => ({ ...v, displayOrder: Number(e.target.value) }))} /></div><Button onClick={createSlide} disabled={creating || !form.imageUrl.trim()} className="min-h-11">{creating && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Guardar slide</Button></div></DialogContent></Dialog>}
      >
        {slides.length === 0 ? <div className="rounded-2xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">Sem slides configurados. A homepage deve apresentar um estado vazio próprio.</div> : <div className="grid gap-4 md:grid-cols-2">{slides.map(slide => <Card key={slide.id} className="overflow-hidden"><div className="aspect-[16/8] bg-muted">{slide.image_url ? <img src={slide.image_url} alt={slide.title || ''} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center"><ImageIcon className="h-8 w-8 text-muted-foreground" /></div>}</div><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><CardTitle className="truncate text-base">{slide.title || 'Sem título'}</CardTitle><CardDescription className="line-clamp-2">{slide.subtitle || 'Sem subtítulo'}</CardDescription></div><Switch checked={slide.is_active} onCheckedChange={enabled => toggleSlide(slide, enabled)} /></div></CardHeader><CardContent className="flex items-center justify-between gap-2 pt-0"><p className="text-xs text-muted-foreground">Ordem {slide.display_order}</p><Button variant="ghost" size="icon" className="h-11 w-11 text-destructive" onClick={() => removeSlide(slide)} aria-label="Eliminar slide"><Trash2 className="h-4 w-4" /></Button></CardContent></Card>)}</div>}
      </DashboardSection>
    </DashboardPage>
  )
}
