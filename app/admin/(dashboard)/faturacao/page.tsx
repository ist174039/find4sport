...
export default async function AdminBillingPage({ searchParams }: { searchParams: Promise<Record<string,string|string[]|undefined>> }) {
  ...
  let txQuery = admin.from('transactions').select('*,user:platform_users(id,full_name,type)', { count:'exact' }).order('created_at',{ascending:false})
  ...
  const mrr = activeSubscriptions.reduce((sum:number, sub:any) => sum + num(sub.plan?.monthly_price),0)
  const marketplaceRows=completedTransactions.filter((tx:any)=>['service_reservation_payment','space_reservation_payment','event_payment','service_package_payment'].includes(String(tx.type)))
  const completedVolume = completedTransactions.reduce((sum:number,tx:any)=>sum+num(tx.gross_amount??tx.amount),0)
  const commissions = marketplaceRows.reduce((sum:number,tx:any)=>sum+num(tx.platform_commission_amount),0)
  const stripeFees = marketplaceRows.reduce((sum:number,tx:any)=>sum+num(tx.stripe_processing_fee_amount),0)
  const platformNet = marketplaceRows.reduce((sum:number,tx:any)=>sum+num(tx.platform_net_amount),0)
