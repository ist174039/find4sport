...
export async function resolveReservationDisputeAction(reservationId: string, resolution: Resolution, note: string) {
  ...
  if (resolution === 'release') {
    ...
    try { await releaseReservationSettlement(reservationId) }
    catch (error) { await db.from('reservations').update({ service_delivery_status:'disputed', settlement_status:'blocked', status:previousStatus, updated_at:new Date().toISOString() }).eq('id', reservationId).eq('settlement_status','eligible'); throw error }
    ...
    await writeAdminAudit(db,{ action:'UPDATE', tableName:'reservations', userEmail:user.email||user.id, message:'Contestação resolvida a favor do prestador e settlement libertado.', data:{ reservation_id:reservationId, resolution:'release', note:cleanNote } })
    await Promise.all([notify(db,reservation.user_id,'A contestação foi analisada. O serviço foi considerado prestado e o pagamento foi libertado ao prestador.',reservationId,'released:athlete'),notify(db,providerId,'A contestação foi resolvida a teu favor e o valor da reserva foi libertado.',reservationId,'released:provider','/dashboard/entregas')])
  } else {
    ...
    if (transaction.stripe_transfer_id) throw new Error('O valor já foi transferido. Este caso exige reversão financeira antes do reembolso.')
    const secret=process.env.STRIPE_SECRET_KEY; if(!secret) throw new Error('Stripe não está configurado.')
    const stripe=new Stripe(secret), chargeId=String(transaction.stripe_charge_id||''), paymentIntentId=String(transaction.stripe_payment_intent_id||'')
    ...
    const {error}=await db.from('reservations').update({payment_status:refundSucceeded?'refunded':'refund_pending',status:'cancelled',service_delivery_status:'cancelled',settlement_status:refundSucceeded?'refunded':'blocked',updated_at:now}).eq('id',reservationId).eq('service_delivery_status','disputed').eq('settlement_status','blocked');if(error)throw error
    ...
    await writeAdminAudit(db,{ action:'UPDATE', tableName:'reservations', userEmail:user.email||user.id, message:'Contestação resolvida com reembolso Stripe.', data:{ reservation_id:reservationId, transaction_id:transaction.id, resolution:'refund', stripe_refund_id:refund.id, stripe_refund_status:refund.status, note:cleanNote } })
    await Promise.all([notify(db,reservation.user_id,refundSucceeded?'A contestação foi analisada e a reserva foi reembolsada.':'A contestação foi analisada e o reembolso da reserva está a ser processado.',reservationId,refundSucceeded?'refunded:athlete':'refund-pending:athlete'),notify(db,providerId,refundSucceeded?'A contestação da reserva foi resolvida com reembolso ao atleta.':'A contestação foi resolvida com reembolso ao atleta, atualmente em processamento.',reservationId,refundSucceeded?'refunded:provider':'refund-pending:provider','/dashboard/entregas')])
  }
  revalidatePath('/admin/disputas');revalidatePath('/admin/faturacao');revalidatePath('/dashboard/agenda');revalidatePath('/dashboard/reservas');revalidatePath('/dashboard/entregas');revalidatePath('/dashboard/confirmacoes')
  return {success:true}
}
