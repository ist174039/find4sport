...
export default function AdminLoginPage() {
  ...
  const unauthorizedMessage = searchParams.get('error') === 'unauthorized'
    ? 'A tua conta não tem permissões de administração.'
    : null
  ...
  const displayedError = error ?? unauthorizedMessage
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loginEmail.toLowerCase() === 'admin') loginEmail = 'admin@find4sport.pt'
    ...
    <div className="min-h-screen bg-slate-50/50 flex flex-col items-center justify-center p-4 relative overflow-hidden select-none font-sans">
      ...
      <div className="w-full max-w-md space-y-8 relative z-10 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="bg-white border border-border/80 rounded-3xl p-6 md:p-8 shadow-xl shadow-slate-100/50 space-y-6">
          {displayedError && (
            <div role="alert" className="p-4 bg-destructive/10 border border-destructive/20 text-destructive rounded-2xl text-xs font-semibold leading-relaxed">
              {displayedError}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="email" type="email" required placeholder="admin@find4sport.pt" value={email} onChange={(e) => setEmail(e.target.value)} className="pl-11 rounded-xl h-12 bg-white border-border/70 text-foreground placeholder:text-muted-foreground/60 focus:border-primary focus:ring-1 focus:ring-primary/45 transition-all text-sm shadow-xs" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? 'text' : 'password'} required placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} className="pl-11 pr-11 rounded-xl h-12 bg-white border-border/70 text-foreground placeholder:text-muted-foreground/60 focus:border-primary focus:ring-1 focus:ring-primary/45 transition-all text-sm shadow-xs" />
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" aria-label={showPassword ? 'Ocultar palavra-passe' : 'Mostrar palavra-passe'}>
                  {showPassword ? <EyeOff className="h-4.5 w-4.5" /> : <Eye className="h-4.5 w-4.5" />}
                </button>
              </div>
            </div>

            <Button type="submit" disabled={loading} className="w-full h-12 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground font-bold transition-all shadow-md shadow-primary/10 text-sm mt-6 cursor-pointer">
              {loading ? <Loader2 className="h-5 w-5 animate-spin mx-auto text-primary-foreground" /> : 'Entrar no Painel'}
            </Button>
          </form>
        </div>

        <div className="text-center">
          <Link href="/" className="text-xs text-muted-foreground hover:text-foreground transition-all inline-flex items-center gap-1 font-medium">Voltar para a Homepage</Link>
        </div>
      </div>
    </div>
  )
