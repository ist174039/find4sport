...
export async function POST(request: Request) {
  try {
    ...
    if (body.eventId) {
      ...
      const { data: event, error: eventError } = await admin.from('events').select('id,title,price_min,status,capacity,start_date,created_by').eq('id', eventId).maybeSingle()
      ...
      if (!event.created_by) return NextResponse.json({ error: 'O evento não tem um organizador de pagamento válido.' }, { status: 409 })
      if (event.created_by === user.id) return NextResponse.json({ error: 'Não podes comprar uma inscrição no teu próprio evento.' }, { status: 400 })

      const baseAmountCents = Math.round(Number(event.price_min || 0) * 100)
      if (!(baseAmountCents > 0)) return NextResponse.json({ error: 'Este evento não requer pagamento.' }, { status: 400 })
      const quote = await resolveMarketplacePaymentQuote(event.created_by, baseAmountCents)
      ...
      if (participantError || !participant) return NextResponse.json({ error: 'Não foi possível iniciar a inscrição no evento.' }, { status: 500 })

      try {
        const metadata = { event_participant_id: participant.id, event_id: eventId, transaction_type: 'event', buyer_user_id: user.id }
        const session = await stripe.checkout.sessions.create({
          mode: 'payment',
          line_items: marketplaceLineItems(quote, `Evento: ${event.title}`),
    ...
    if (serviceId) {
      ...
      const { data: provider } = await admin.from('professionals').select('user_id,stripe_account_id,status').eq('id', service.professional_id).maybeSingle()
      if (!provider || provider.status !== 'active') return NextResponse.json({ error: 'O profissional não está disponível para receber reservas.' }, { status: 409 })
      if (provider.user_id === user.id) return NextResponse.json({ error: 'Não podes reservar o teu próprio serviço.' }, { status: 400 })
      ...
      baseAmountCents = Math.round(Number(service.price || 0) * 100)
      ...
      destinationAccountId = provider.stripe_account_id || null
      ...
      const { data: space } = await admin.from('sport_spaces').select('id,owner_user_id,stripe_account_id,status').eq('id', room.space_id).maybeSingle()
      if (!space || !['active', 'published'].includes(String(space.status))) return NextResponse.json({ error: 'O espaço não está disponível para receber reservas.' }, { status: 409 })
      if (space.owner_user_id === user.id) return NextResponse.json({ error: 'Não podes reservar o teu próprio espaço.' }, { status: 400 })
      if (spaceId && room.space_id !== spaceId) return NextResponse.json({ error: 'Sala/campo inválido para este espaço.' }, { status: 400 })
      baseAmountCents = Math.round(Number(room.price_per_hour || 0) * 100)
      title = room.name || 'Sala/Campo'
      resolvedSpaceId = room.space_id
      resolvedRoomId = room.id
      providerUserId = space.owner_user_id
      destinationAccountId = space.stripe_account_id || null
      cancelReturn = `/espacos/${room.space_id}`
    }

    if (!(baseAmountCents > 0)) return NextResponse.json({ error: 'Esta reserva não requer pagamento online.' }, { status: 400 })
    if (!providerUserId) return NextResponse.json({ error: 'Prestador de pagamento inválido.' }, { status: 409 })
    const quote = await resolveMarketplacePaymentQuote(providerUserId, baseAmountCents, destinationAccountId)
    ...
    if (resolvedProfessionalId) {
      ...
      if (!(slots || []).some(slot => startTime >= String(slot.start_time).slice(0, 5) && endTime <= String(slot.end_time).slice(0, 5))) return NextResponse.json({ error: 'O horário escolhido está fora da disponibilidade do profissional.' }, { status: 409 })
    }
    if (resolvedRoomId) {
      ...
      if (!(slots || []).some(slot => startTime >= String(slot.start_time).slice(0, 5) && endTime <= String(slot.end_time).slice(0, 5))) return NextResponse.json({ error: 'O horário escolhido está fora da disponibilidade da sala/campo.' }, { status: 409 })
    }
    ...
    const reservationPayload: Record<string, unknown> = {
      ...
      end_time: endTime,
      amount: quote.totalAmountCents / 100,
      status: 'pending',
    ...
    if (reservationResult.error || !reservationResult.data) return NextResponse.json({ error: reservationResult.error?.message ? `Não foi possível criar a reserva: ${reservationResult.error.message}` : 'Não foi possível criar a reserva.' }, { status: 500 })
    const reservation = reservationResult.data

    try {
      const transactionType = resolvedRoomId ? 'space_reservation' : 'service_reservation'
      const metadata = { reservation_id: reservation.id, transaction_type: transactionType, buyer_user_id: user.id, ...(resolvedRoomId ? { space_room_id: resolvedRoomId } : {}) }
      const session = await stripe.checkout.sessions.create({
        mode: 'payment',
        line_items: marketplaceLineItems(quote, `Reserva: ${title}`, `Data: ${bookingDate.toLocaleDateString('pt-PT')} · ${startTime}–${endTime}`),
        payment_intent_data: marketplacePaymentIntentData(quote, metadata),
