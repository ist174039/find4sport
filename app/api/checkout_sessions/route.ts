...
function getBaseUrl(request: Request) {
  ...
  if (host) {
    try { return new URL(`${protocol}://${host}`).origin } catch {}
  }
  const configured = process.env.NEXT_PUBLIC_SITE_URL?.trim()
  if (configured) {
    try { return new URL(/^https?:\/\//i.test(configured) ? configured : `https://${configured}`).origin } catch {}
  }
  ...
  try {
    ...
    if (body.eventId) {
      ...
      const { data: event, error: eventError } = await admin
        .from('events')
        .select('id,title,price_min,status,capacity,start_date')
        .eq('id', eventId)
        .maybeSingle()
      ...
      const { data: participant, error: participantError } = await admin
        ...
        .insert({ event_id: eventId, user_id: user.id, status: 'pending', payment_status: 'pending' })
        .select('id')
        .single()
    ...
    if (resolvedProfessionalId) overlap = overlap.eq('professional_id', resolvedProfessionalId)
    if (resolvedRoomId) overlap = overlap.eq('space_room_id', resolvedRoomId)
    const { data: overlapping, error: overlapError } = await overlap.limit(1)
    if (overlapError) return NextResponse.json({ error: 'Não foi possível validar conflitos de agenda.' }, { status: 500 })
    if (overlapping?.length) return NextResponse.json({ error: 'Este horário já não está disponível.' }, { status: 409 })
    ...
    if (reservationError || !reservation) {
      console.error('Paid reservation insert error:', reservationError)
    ...
    try {
      const session = await stripe.checkout.sessions.create({
        ...
        metadata: { reservation_id: reservation.id, transaction_type: 'reservation' },
