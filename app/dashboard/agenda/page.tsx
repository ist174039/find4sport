...
export default async function DashboardAgendaPage(){
  ...
  const activeParticipations=(myParticipations||[]).filter(participation=>participation.status!=='cancelled')
  const serviceIds=[...new Set((myReservations||[]).map((r:any)=>r.service_id).filter(Boolean))] as string[];const professionalIds=[...new Set((myReservations||[]).map((r:any)=>r.professional_id).filter(Boolean))] as string[];const spaceIds=[...new Set((myReservations||[]).map((r:any)=>r.space_id).filter(Boolean))] as string[];const roomIds=[...new Set((myReservations||[]).map((r:any)=>r.space_room_id).filter(Boolean))] as string[];const eventIds=[...new Set(activeParticipations.map(p=>p.event_id))]
  ...
  const eventMap=new Map<string,any>(((myEvents as any).data||[]).map((event:any)=>[event.id,event]))
  items.push(...activeParticipations.flatMap(participation=>{const event:any=eventMap.get(participation.event_id);if(!event)return[];return[{id:`my-event-${event.id}`,kind:'event' as const,title:`Vou participar · ${event.title}`,at:event.start_date,endAt:event.end_date||null,status:participation.status||'pending',location:event.address,href:`/eventos/${event.id}`}]}))
