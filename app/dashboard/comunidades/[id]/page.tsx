...
const postingPolicyOptions = [
  ['members', 'Todos os membros podem publicar'],
  ['reactions_only', 'Administradores publicam; membros acompanham e reagem'],
  ['admin_only', 'Só administradores publicam'],
] as const
...
export default async function ManageCommunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <DashboardPage>
    ...
    <DashboardSection title="Informação, descoberta e regras" description="Mantém a página pública e as regras da comunidade coerentes com a forma como ela funciona.">
      <form action={updateCommunityAction.bind(null, community.id)} className="grid gap-5">
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-2"><Label htmlFor="location_scope">Âmbito</Label><select id="location_scope" name="location_scope" defaultValue={community.location_scope || 'online'} className="min-h-11 w-full rounded-xl border border-input bg-background px-3 text-sm">{scopeOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><p className="text-xs text-muted-foreground">Se mudares para Online, a localização e as coordenadas guardadas são removidas.</p></div>
          <div className="space-y-2"><Label htmlFor="address">Localização</Label><div className="relative"><MapPin className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-muted-foreground" /><Input id="address" name="address" defaultValue={community.address || ''} className="min-h-11 pl-10 text-base" placeholder="Lisboa, Portugal" /></div><p className="text-xs text-muted-foreground">Usada para comunidades locais, regionais ou nacionais.</p></div>
        </div>
