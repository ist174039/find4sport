...
export default async function ManageCommunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [communityResult, membersResult, postsResult, requestsResult, mediaResult] = await Promise.all([
  ...
  return (
    <DashboardPage>
      <DashboardPageHeader
        ...
        description="Gere informação, pedidos de adesão, membros, publicações e galeria da comunidade."
        action={<div className="flex gap-2"><Button asChild variant="outline"><Link href="/dashboard/comunidades"><ArrowLeft className="mr-2 h-4 w-4" />Comunidades</Link></Button><Button asChild><Link href={`/comunidades/${community.slug || community.id}`}>Ver página pública</Link></Button></div>}
      />

      <DashboardStatGrid>
        <DashboardStat label="Membros" value={members.length} icon={<Users className="h-5 w-5" />} />
        <DashboardStat label="Pedidos" value={requests.length} icon={<ShieldCheck className="h-5 w-5" />} />
        <DashboardStat label="Publicações" value={posts.length} icon={<MessageSquare className="h-5 w-5" />} />
        <DashboardStat label="Imagens" value={media.length} icon={<Camera className="h-5 w-5" />} />
      </DashboardStatGrid>
      ...
      <DashboardSection title="Informação pública" description="Tudo o que for guardado aqui aparece na página pública da comunidade.">
        <form action={updateCommunityAction.bind(null, community.id)} className="grid gap-4">
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-2"><Label htmlFor="name">Nome</Label><Input id="name" name="name" defaultValue={community.name || ''} className="min-h-11 text-base" required /></div>
            <div className="space-y-2"><Label htmlFor="sport_category">Modalidade / tema</Label><Input id="sport_category" name="sport_category" defaultValue={community.sport_category || ''} className="min-h-11 text-base" /></div>
          </div>
      ...
      <DashboardSection title="Media" description="Galeria própria da comunidade. Imagens não são guardadas na base de dados; ficam no Supabase Storage.">
        <form action={uploadCommunityMediaAction.bind(null, community.id)} className="grid gap-3 rounded-xl border border-dashed border-border p-4 sm:grid-cols-[1fr_1fr_auto] sm:items-end" encType="multipart/form-data">
          <div className="space-y-2"><Label htmlFor="file">Imagem</Label><Input id="file" name="file" type="file" accept="image/jpeg,image/png,image/webp" className="min-h-11 text-base" required /></div>
          <div className="space-y-2"><Label htmlFor="caption">Legenda</Label><Input id="caption" name="caption" className="min-h-11 text-base" /></div>
          <Button type="submit" className="min-h-11">Adicionar</Button>
        </form>
