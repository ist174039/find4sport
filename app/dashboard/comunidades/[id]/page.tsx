...
import { ArrowLeft, Crown, Globe, Lock, MapPin, MessageSquare, ShieldCheck, Trash2, UserMinus, Users } from 'lucide-react'
...
import { CommunityCoverUploader } from '@/components/community-cover-uploader'
...
import { deleteCommunityPostAction, removeCommunityMemberAction, setCommunityMemberRoleAction, updateCommunityAction } from '../actions'

const scopeOptions = [['online', 'Online'], ['local', 'Local'], ['regional', 'Regional'], ['national', 'Nacional']] as const
const postingPolicyOptions = [['members', 'Todos os membros podem publicar'], ['reactions_only', 'Administradores publicam; membros acompanham e reagem'], ['admin_only', 'Só administradores publicam']] as const
...
export default async function ManageCommunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [communityResult, membersResult, postsResult, requestsResult, categoriesResult] = await Promise.all([
  ...
  if (categoriesResult.error) throw new Error('Não foi possível carregar as modalidades.')
  const members = membersResult.data || []
  const posts = postsResult.data || []
  const requests = requestsResult.error ? [] : (requestsResult.data || [])
  const adminCount = members.filter(member => member.role === 'admin').length
  const categories: TaxonomyOption[] = categoriesResult.data ?? []
  ...
  return <DashboardPage>
    <DashboardPageHeader title={community.name} description="Gere informação, acesso, administradores, membros e publicações da comunidade." action={<div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href="/dashboard/comunidades"><ArrowLeft className="mr-2 h-4 w-4" />Comunidades</Link></Button><Button asChild><Link href={`/comunidades/${community.slug || community.id}`}>Ver página pública</Link></Button></div>} />
    <DashboardStatGrid><DashboardStat label="Membros" value={members.length} icon={<Users className="h-5 w-5" />} /><DashboardStat label="Administradores" value={adminCount} icon={<Crown className="h-5 w-5" />} /><DashboardStat label="Pedidos" value={requests.length} icon={<ShieldCheck className="h-5 w-5" />} /><DashboardStat label="Publicações" value={posts.length} icon={<MessageSquare className="h-5 w-5" />} /></DashboardStatGrid>
    <DashboardSection title="Imagem de capa" description="A imagem aparece no banner da página pública da comunidade."><CommunityCoverUploader communityId={community.id} userId={user.id} initialUrl={community.cover_url} /></DashboardSection>
    <DashboardSection title="Informação, descoberta e regras" description="Mantém a página pública e as regras da comunidade coerentes com a forma como ela funciona."><form action={updateCommunityAction.bind(null, community.id)} className="grid gap-5">
      ...
      <div className="grid gap-4 lg:grid-cols-2"><div className="space-y-2"><Label htmlFor="location_scope">Âmbito</Label><select id="location_scope" name="location_scope" defaultValue={community.location_scope || 'online'} className="min-h-11 w-full rounded-xl border border-input bg-background px-3 text-sm">{scopeOptions.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><p className="text-xs text-muted-foreground">Se mudares para Online, a localização e as coordenadas guardadas são removidas.</p></div><div className="space-y-2"><Label htmlFor="address">Localização</Label><div className="relative"><MapPin className="pointer-events-none absolute left-3 top-3.5 h-4 w-4 text-muted-foreground" /><Input id="address" name="address" defaultValue={community.address || ''} className="min-h-11 pl-10 text-base" placeholder="Lisboa, Portugal" /></div><p className="text-xs text-muted-foreground">Usada para comunidades locais, regionais ou nacionais.</p></div></div>
      <div className="space-y-2"><Label htmlFor="posting_policy">Quem pode publicar?</Label><select id="posting_policy" name="posting_policy" defaultValue={community.posting_policy || 'members'} className="min-h-11 w-full rounded-xl border border-input bg-background px-3 text-sm">{postingPolicyOptions.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div>
      ...
      <div><Button type="submit" className="min-h-11 w-full sm:w-auto">Guardar alterações</Button></div>
    </form></DashboardSection>
    ...
    <DashboardSection title="Membros e administração" description="Promove membros de confiança a administradores antes de saíres da comunidade; a comunidade nunca pode ficar sem administração."><div className="grid gap-3 md:grid-cols-2">{members.map((member:any)=><article key={member.id} className="flex flex-col gap-3 rounded-xl border border-border p-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full bg-primary/10 font-bold text-primary">{member.platform_users?.avatar_url?<img src={member.platform_users.avatar_url} alt="" className="h-full w-full object-cover"/>:member.platform_users?.full_name?.charAt(0)||'U'}</div><div className="min-w-0"><p className="truncate text-sm font-semibold">{member.platform_users?.full_name||'Utilizador'}</p><p className="flex items-center gap-1 text-xs text-muted-foreground">{member.role==='admin'&&<Crown className="h-3.5 w-3.5 text-primary"/>}{member.role==='admin'?'Administrador':'Membro'}{member.user_id===user.id?' · Tu':''}</p></div></div>{member.user_id!==user.id&&<div className="flex gap-2"><form action={setCommunityMemberRoleAction.bind(null,community.id,member.user_id,member.role==='admin'?'member':'admin')}><Button type="submit" variant="outline" size="sm" className="min-h-10">{member.role==='admin'?'Retirar admin':'Tornar admin'}</Button></form><form action={removeCommunityMemberAction.bind(null,community.id,member.user_id)}><Button type="submit" variant="ghost" size="icon" className="h-10 w-10 text-destructive" aria-label="Remover membro"><UserMinus className="h-4 w-4"/></Button></form></div>}</article>)}</div></DashboardSection>
