...
import { ArrowLeft, Globe, Lock, MessageSquare, ShieldCheck, Trash2, UserMinus, Users } from 'lucide-react'
...
import { deleteCommunityPostAction, removeCommunityMemberAction, updateCommunityAction } from '../actions'
...
export default async function ManageCommunityPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <DashboardPage>
    <DashboardPageHeader title={community.name} description="Gere informação, membros, publicações e imagem pública da comunidade." action={<div className="flex gap-2"><Button asChild variant="outline"><Link href="/dashboard/comunidades"><ArrowLeft className="mr-2 h-4 w-4" />Comunidades</Link></Button><Button asChild><Link href={`/comunidades/${community.slug || community.id}`}>Ver página pública</Link></Button></div>} />

    <DashboardStatGrid><DashboardStat label="Membros" value={members.length} icon={<Users className="h-5 w-5" />} /><DashboardStat label="Pedidos" value={requests.length} icon={<ShieldCheck className="h-5 w-5" />} /><DashboardStat label="Publicações" value={posts.length} icon={<MessageSquare className="h-5 w-5" />} /></DashboardStatGrid>
