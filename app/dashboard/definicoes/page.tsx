...
import { Bell, CreditCard, FileText, UserRound } from 'lucide-react'
...
export default async function SettingsPage() {
  ...
  return (
    <DashboardPage>
      <DashboardPageHeader title="Definições" description="Segurança, preferências e lifecycle real da conta." />

      <DashboardSection title="Segurança" description="A alteração de password é aplicada diretamente à conta Supabase Auth autenticada."><SecuritySettings /></DashboardSection>
      ...
      <DashboardSection title="Conta e preferências" description="Cada opção abre o módulo responsável; não duplicamos configurações em vários ecrãs.">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <Button asChild variant="outline" className="min-h-16 h-auto justify-start gap-3 p-4"><Link href="/privacidade"><FileText className="h-5 w-5 text-primary" /><span className="min-w-0 text-left"><span className="block font-semibold">Privacidade e RGPD</span><span className="block break-words text-xs font-normal text-muted-foreground">Consultar a política de proteção de dados</span></span></Link></Button>
        </div>
      </DashboardSection>

      <DashboardSection title="Lifecycle da conta" description="Desativação reversível e eliminação condicionada pelo histórico transacional. Não apagamos reservas ou pagamentos em cascata apenas para satisfazer um clique.">
