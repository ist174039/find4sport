...
type EventFormState = { title:string; description:string; category_id:string; address:string; start_date:string; end_date:string; capacity:string; price_min:string; price_max:string }
const initialForm: EventFormState = { title:'', description:'', category_id:'', address:'', start_date:'', end_date:'', capacity:'', price_min:'', price_max:'' }

export function EventCreateForm({ categories, paidEventsEnabled }: { categories: TaxonomyOption[]; paidEventsEnabled: boolean }) {
  ...
  useEffect(() => () => { if (bannerPreview) URL.revokeObjectURL(bannerPreview); galleryPreviews.forEach(URL.revokeObjectURL) }, [bannerPreview,galleryPreviews])
  const change = (key:keyof EventFormState,value:string) => setForm(current=>({...current,[key]:value}))
  const setBanner = (file:File|null) => { if(bannerPreview) URL.revokeObjectURL(bannerPreview); setBannerFile(file); setBannerPreview(file?URL.createObjectURL(file):null) }
  const addGallery = (files:File[]) => { const next=[...galleryFiles,...files].slice(0,12); galleryPreviews.forEach(URL.revokeObjectURL); setGalleryFiles(next); setGalleryPreviews(next.map(URL.createObjectURL)) }
  const removeGallery = (index:number) => { const next=galleryFiles.filter((_,i)=>i!==index); galleryPreviews.forEach(URL.revokeObjectURL); setGalleryFiles(next); setGalleryPreviews(next.map(URL.createObjectURL)) }

  const submit = async (event:React.FormEvent) => {
    event.preventDefault(); if(saving)return; setSaving(true)
    try {
      const data=new FormData(); Object.entries(form).forEach(([key,value])=>data.set(key,value)); if(bannerFile)data.set('banner',bannerFile); galleryFiles.forEach(file=>data.append('gallery',file))
    ...
    } catch(error) { showAlert('Não foi possível criar o evento',error instanceof Error?error.message:'Erro inesperado.','error') }
  ...
  return <div className="mx-auto max-w-4xl space-y-5">
    ...
    <Card className="overflow-visible rounded-3xl"><CardHeader className="border-b"><CardTitle>Criar evento</CardTitle><CardDescription>Preenche apenas informação real. Publicação e permissões são validadas no servidor.</CardDescription></CardHeader><CardContent className="p-4 sm:p-6"><form onSubmit={submit} className="space-y-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2"><label className="relative z-20 space-y-2"><Label>Modalidade</Label><TaxonomyCombobox options={categories} value={form.category_id} onChange={value=>change('category_id',String(value))} placeholder="Pesquisar modalidade"/></label><label className="space-y-2"><Label>Localização</Label><Input value={form.address} onChange={event=>change('address',event.target.value)} maxLength={300} className="min-h-11"/></label></div>
      ...
      <div className="grid gap-4 sm:grid-cols-3"><label className="space-y-2"><Label>Capacidade</Label><Input type="number" min={1} max={100000} value={form.capacity} onChange={event=>change('capacity',event.target.value)} className="min-h-11"/></label><label className="space-y-2"><Label>Preço mínimo (€)</Label><Input type="number" min={0} step="0.01" value={form.price_min} onChange={event=>change('price_min',event.target.value)} disabled={!paidEventsEnabled} className="min-h-11"/></label><label className="space-y-2"><Label>Preço máximo (€)</Label><Input type="number" min={0} step="0.01" value={form.price_max} onChange={event=>change('price_max',event.target.value)} disabled={!paidEventsEnabled} className="min-h-11"/></label></div>
      {!paidEventsEnabled&&<div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm">Os eventos pagos estão temporariamente desativados pela administração. Este evento será gratuito.</div>}
      {(Number(form.price_min)>0||Number(form.price_max)>0)&&<div className="rounded-xl border border-primary/20 bg-primary/5 p-3 text-sm text-primary">Comissões e taxas são determinadas pelo plano ativo e pelo checkout Stripe.</div>}
