...
export default function FaturacaoPage(){
  ...
  useEffect(()=>{void (async()=>{
    try{
      ...
      if(role){
        const [{data,error},connectRes]=await Promise.all([
          supabase.from('subscription_plans').select('*,entitlements:plan_entitlements(*)').eq('audience',role).eq('is_active',true).eq('is_public',true).order('sort_order'),
          fetch('/api/stripe/connect',{method:'GET',cache:'no-store'}),
        ])
        if(error)throw error; setPlans((data||[]) as Plan[])
        if(connectRes.ok){const status=await connectRes.json(); setConnectStatus(status as ConnectStatus)}
      }
