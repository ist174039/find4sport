  ...
  return (
    <DashboardPage className="min-w-0 max-w-full">
      <DashboardPageHeader
        ...
        action={publicProfileHref ? <Button asChild variant="outline" className="min-h-11 max-w-full"><Link href={publicProfileHref} target={access.role === 'professional' ? '_blank' : undefined} className="min-w-0"><span className="truncate">{access.role === 'professional' ? 'Ver perfil público' : 'Gerir espaço'}</span><ExternalLink className="ml-2 h-4 w-4 shrink-0" /></Link></Button> : undefined}
      />
      ...
      <DashboardSection title="Imagem e capa" description="JPEG, PNG ou WebP até 5 MB. Os ficheiros são guardados no Storage; a base de dados guarda apenas os URLs.">
        <div className="grid min-w-0 gap-5 md:grid-cols-[180px_minmax(0,1fr)]">
          <div className="min-w-0 space-y-3">
            ...
            <form action={uploadAvatarAction} className="min-w-0 space-y-2"><Input type="file" name="file" accept="image/jpeg,image/png,image/webp" required className="min-h-11 w-full max-w-full text-base" /><Button type="submit" variant="outline" className="min-h-11 w-full"><Camera className="mr-2 h-4 w-4" />Atualizar foto</Button></form>
          </div>
          <div className="min-w-0 space-y-3">
            <div className="aspect-[3/1] min-h-32 min-w-0 overflow-hidden rounded-2xl border border-border bg-muted">{bannerUrl ? <img src={bannerUrl} alt="Capa" className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Sem imagem de capa</div>}</div>
            <form action={uploadBannerAction} className="grid min-w-0 gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><Input type="file" name="file" accept="image/jpeg,image/png,image/webp" required className="min-h-11 w-full max-w-full text-base" /><Button type="submit" variant="outline" className="min-h-11">Atualizar capa</Button></form>
          </div>
        </div>
      </DashboardSection>
      ...
      <DashboardSection title="Informação do perfil" description="A informação é guardada no servidor e sincronizada com a identidade da conta.">
        <form action={updateProfileAction} className="min-w-0 space-y-5">
          ...
          {access.role === 'professional' && professional && <>
            ...
            {(categories || []).length > 0 && <div className="min-w-0 space-y-2"><Label>Modalidades</Label><div className="grid min-w-0 gap-2 sm:grid-cols-2 lg:grid-cols-3">{(categories || []).map((category: any) => <label key={category.id} className="flex min-h-12 min-w-0 cursor-pointer items-center gap-3 rounded-xl border border-border px-3 py-2 text-sm"><input type="checkbox" name="category_ids" value={category.id} defaultChecked={selectedCategoryIds.includes(category.id)} className="h-5 w-5 shrink-0" /><span className="min-w-0 break-words">{category.emoji ? `${category.emoji} ` : ''}{category.name}</span></label>)}</div></div>}
          </>}
      ...
      {access.role === 'professional' && professional && <>
        <DashboardSection title="Qualificações" description="Certificações e formação associadas ao perfil profissional."><QualificationsManager professionalId={professional.id} /></DashboardSection>
