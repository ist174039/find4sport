...
const PAGE_SIZE = 20
type ReservationStatus = NonNullable<Tables<'reservations'>['status']>
...
export default async function ReservasPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const status = parseStatus(firstParam(params.status) || 'all')
  ...
  const ids = (reservationRows || []).map((row: any) => row.id)
  const admin = createAdminClient()
  const changeRows = ids.length ? ((await (admin as any).from('reservation_change_requests').select('id,reservation_id,requested_date,requested_start_time,requested_end_time,status').in('reservation_id', ids).eq('status', 'pending')).data || []) : []
  const changeByReservation = new Map(changeRows.map((row: any) => [row.reservation_id, row]))
  const items = (reservationRows || []).map((row: any) => ({ ...row, package_session_consumed: Boolean(row.package_session_consumed), changeRequest: changeByReservation.get(row.id) || null }) as ReservationListItem)

  return <ReservationsClient key={`${query}:${status}`} role={access.role} data={{ items, total, page, pageSize: PAGE_SIZE, query, status }} initialAvailability={availability} />
}
