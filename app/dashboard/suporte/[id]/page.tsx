...
export default async function DashboardSupportDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  const adminIds=[...new Set((messages||[]).map((m:any)=>m.sender_admin_id).filter(Boolean))];const adminResult=adminIds.length?await db.from('admins').select('id,email').in('id',adminIds):{data:[]};const admins=new Map<string,string>((adminResult.data||[]).map((a:any)=>[String(a.id),String(a.email||'Administrador')] as [string,string]))
