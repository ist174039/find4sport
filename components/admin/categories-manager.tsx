...
export function CategoriesManager({initialCategories}:{initialCategories:TaxonomyCategory[]}){
 ...
 function openEdit(c:TaxonomyCategory){setEditingId(c.id);setForm({name:c.name,slug:c.slug,emoji:c.emoji||'',color:c.color||'#14b8a6',code:c.code||'',taxonomy_type:c.taxonomy_type,is_active:c.is_active,parent_id:c.parent_id||''});setDialogOpen(true)}
 ...
 async function save(){if(!form.name.trim())return;setSaving(true);try{const payload={...form,parent_id:form.parent_id||null};if(editingId){const u=await updateCategoryAction(editingId,payload);setCategories(p=>p.map(c=>c.id===editingId?u as TaxonomyCategory:c))}else{const c=await createCategoryAction(payload);setCategories(p=>[...p,c as TaxonomyCategory])}setDialogOpen(false);showAlert('Taxonomia guardada','A configuração foi atualizada.','success')}catch(e){showAlert('Erro',e instanceof Error?e.message:'Não foi possível guardar.','error')}finally{setSaving(false)}}
 async function toggle(c:TaxonomyCategory){try{const u=await setCategoryActiveAction(c.id,!c.is_active);setCategories(p=>p.map(x=>x.id===c.id?u as TaxonomyCategory:x))}catch(e){showAlert('Erro',e instanceof Error?e.message:'Não foi possível alterar o estado.','error')}}
 async function remove(c:TaxonomyCategory){if(!await showConfirm('Eliminar entrada',`Eliminar “${c.name}”? Prefere desativar quando existirem relações.`,{confirmLabel:'Eliminar',destructive:true}))return;try{await deleteCategoryAction(c.id);setCategories(p=>p.filter(x=>x.id!==c.id))}catch(e){showAlert('Não foi possível eliminar',e instanceof Error?e.message:'Erro inesperado.','error')}}
