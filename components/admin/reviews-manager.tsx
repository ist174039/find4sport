...
export function ReviewsManager({ initialReviews }: { initialReviews: ReviewRow[] }) {
  ...
  return (
    <div className="min-w-0 space-y-5">
      ...
      <div className="rounded-2xl border bg-card p-3 sm:p-4"><div className="flex flex-col gap-3 sm:flex-row"><label className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input type="search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Pesquisar autor, entidade ou comentário..." className="h-11 w-full rounded-xl border bg-background pl-10 pr-3 text-base outline-none focus:border-primary focus:ring-2 focus:ring-primary/15" /></label><div className="grid grid-cols-3 gap-2 sm:flex">{([['all', 'Todas'], ['high', '4–5 ★'], ['low', '1–2 ★']] as const).map(([value, label]) => <Button key={value} type="button" variant={filter === value ? 'default' : 'outline'} onClick={() => setFilter(value)} className="min-h-11">{label}</Button>)}</div></div></div>
