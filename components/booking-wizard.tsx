...
export function BookingWizard({ open, onOpenChange, service, professionalId, spaceId }: BookingWizardProps) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (spaceId) {
        ...
        if (error) {
          setRooms([])
          setErrorMsg('Não foi possível carregar as salas/campos deste espaço.')
          return
        }
      ...
      if (professionalId) {
        ...
        const packagesPromise = service?.id
          ? fetch(`/api/services/${encodeURIComponent(service.id)}/package-credits`, { cache: 'no-store' }).then(response => response.ok ? response.json() as Promise<PackagePayload> : { purchases: [] }).catch(() => ({ purchases: [] }))
          : Promise.resolve<PackagePayload>({ purchases: [] })
        const [availabilityResult, packagePayload] = await Promise.all([availabilityPromise, packagesPromise])
        if (cancelled) return
        if (availabilityResult.error) {
          setAvailability([])
          setErrorMsg('Não foi possível carregar a disponibilidade.')
        } else {
        ...
        if (credits.length > 0) {
          setUsePackage(true)
          setSelectedPackagePurchaseId(credits[0].id)
        }
      }
    })()
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (error) {
        setAvailability([])
        setErrorMsg('Não foi possível carregar a disponibilidade desta sala/campo.')
        return
      }
  ...
  const validateDateTime = () => {
    ...
    if (!slots.some(slot => selectedTime >= String(slot.start_time).slice(0, 5) && computedEndTime <= String(slot.end_time).slice(0, 5))) { setErrorMsg(`A reserva de ${durationMinutes} min tem de ficar dentro de um período disponível.`); return false }
    setErrorMsg('')
    return true
  }
  ...
  const handleNext = () => {
    if (step === 1 && spaceId) {
      if (!selectedRoom) { setErrorMsg('Seleciona uma sala/campo.'); return }
      setErrorMsg('')
      setStep(2)
  ...
  const handleCheckout = async () => {
    ...
    setErrorMsg('')
    try {
      if (!spaceId && service?.id && professionalId && usePackage && selectedPackagePurchaseId) {
        await createPackageReservationAction({ serviceId: service.id, professionalId, packagePurchaseId: selectedPackagePurchaseId, date: selectedDate, startTime: selectedTime })
        router.push('/dashboard/agenda?booking=package')
        return
      }
      ...
      if (isFree) {
        ...
        router.push('/dashboard/agenda?booking=success')
        return
      }
  ...
  return <Dialog open={open} onOpenChange={handleOpenChange}><DialogContent className="flex h-[100dvh] w-screen max-w-none flex-col overflow-hidden rounded-none p-0 sm:h-auto sm:max-h-[90dvh] sm:w-full sm:max-w-lg sm:rounded-3xl">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-6">{errorMsg && <div className="mb-4 rounded-xl border border-destructive/20 bg-destructive/10 p-3 text-sm text-destructive">{errorMsg}</div>}
      ...
      {step === 2 && <div className="space-y-5"><div className="space-y-2"><Label>Data</Label><div className="flex justify-center rounded-2xl border p-3"><Calendar mode="single" selected={selectedDate ? new Date(`${selectedDate}T12:00:00`) : undefined} onSelect={date => { if (!date) { setSelectedDate(''); return } setSelectedDate(`${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`); setSelectedTime(''); setErrorMsg('') }} disabled={date => { const today = new Date(); today.setHours(0, 0, 0, 0); return date < today || !availability.some(slot => slot.day_of_week === date.getDay()) }} locale={pt} className="p-0" /></div></div>{selectedDate && <div className="rounded-2xl border bg-muted/30 p-3 text-sm"><div className="mb-2 flex items-center gap-2 font-semibold"><CalendarIcon className="h-4 w-4 text-primary" />Períodos disponíveis</div>{availability.filter(slot => slot.day_of_week === new Date(`${selectedDate}T12:00:00`).getDay()).map((slot, index) => <p key={`${slot.day_of_week}-${slot.start_time}-${index}`} className="text-muted-foreground">{String(slot.start_time).slice(0, 5)}–{String(slot.end_time).slice(0, 5)}</p>)}</div>}<div className="space-y-2"><Label htmlFor="booking-time">Hora de início</Label><div className="relative"><Clock className="absolute left-3 top-3.5 h-4 w-4 text-muted-foreground" /><Input id="booking-time" type="time" value={selectedTime} onChange={event => { setSelectedTime(event.target.value); setErrorMsg('') }} className="min-h-12 pl-9 text-base" disabled={!selectedDate} /></div>{selectedTime && computedEndTime && <p className="text-xs text-muted-foreground">Fim previsto: <strong>{computedEndTime}</strong></p>}</div></div>}
