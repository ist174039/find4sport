...
export function ChatInterface({ initialContacts, initialMessages, currentUserId }: { initialContacts: Contact[]; initialMessages: Message[]; currentUserId: string }) {
  ...
  const activeMessages = messages.filter(message => {
    ...
    return (message.sender_id === currentUserId && message.receiver_id === activeContact.userId) || (message.receiver_id === currentUserId && message.sender_id === activeContact.userId)
  }).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime())

  const filteredContacts = contacts.filter(contact => {
    const q=contactSearch.trim().toLowerCase()
    return !q || `${contact.name} ${contact.role} ${contact.lastMsg} ${contact.contextLabel||''} ${contact.contextDetail||''}`.toLowerCase().includes(q)
  }).sort((a,b)=>Number(Boolean(a.archived))-Number(Boolean(b.archived)) || new Date(b.lastMsgDate).getTime()-new Date(a.lastMsgDate).getTime())
  ...
  useEffect(() => {
    ...
    void markAsRead(unreadMessages.map(message => message.id)).then(() => {
      ...
      setContacts(previous => previous.map(contact => contact.id === activeContact.id ? { ...contact, unread: 0 } : contact))
    })
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('messages_changes').on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages', filter: `receiver_id=eq.${currentUserId}` }, payload => {
      ...
      setContacts(previous => previous.map(contact => {
        const belongsToThread = Boolean(newMsg.thread_id && contact.threadId === newMsg.thread_id)
        const belongsToLegacyPair = !newMsg.thread_id && !contact.threadId && contact.userId === newMsg.sender_id
        if (!belongsToThread && !belongsToLegacyPair) return contact
        return { ...contact, unread: activeConversationId === contact.id ? 0 : contact.unread + 1, lastMsg: newMsg.content, lastMsgDate: newMsg.created_at }
      }))
    }).subscribe()
    return () => { void supabase.removeChannel(channel) }
  }, [currentUserId, activeConversationId])

  const selectContact = (conversationId: string) => { setActiveConversationId(conversationId); setMobileChatOpen(true) }
  const handleSend = async (event: React.FormEvent) => {
    event.preventDefault()
    const content = newMessage.trim()
    if (!content || !activeContact || isSending || activeContact.archived) return
    setIsSending(true)
    try {
  ...
  const formatDayHeader = (dateStr: string) => { const date=new Date(dateStr); if(isToday(date))return'Hoje'; if(isYesterday(date))return'Ontem'; return format(date,"dd 'de' MMMM",{locale:pt}) }
  ...
  return <div className="mx-auto flex h-full min-h-0 w-full max-w-6xl overflow-hidden bg-card md:gap-6 md:rounded-xl md:bg-transparent">
    <aside className={`${mobileChatOpen?'hidden md:flex':'flex'} w-full flex-col overflow-hidden bg-card md:w-80 md:shrink-0 md:rounded-xl md:border md:border-border lg:w-96`}>
      <div className="shrink-0 border-b border-border bg-muted/20 p-4"><div className="mb-3"><h2 className="flex items-center gap-2 text-lg font-bold">Conversas{unreadTotal>0&&<span className="rounded-full bg-primary px-2 py-0.5 text-xs font-bold text-primary-foreground">{unreadTotal}</span>}</h2><p className="mt-0.5 text-xs text-muted-foreground">Cada conversa pertence a uma reserva ou evento pago. Históricos encerrados ficam em consulta.</p></div><label className="relative block"><span className="sr-only">Pesquisar conversas</span><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/><input type="search" inputMode="search" placeholder="Pessoa, serviço ou evento..." value={contactSearch} onChange={event=>setContactSearch(event.target.value)} className="h-11 w-full rounded-xl border border-border bg-background pl-10 pr-4 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"/></label></div>
      <div className="flex-1 space-y-1 overflow-y-auto overscroll-contain p-2">{filteredContacts.length===0?<div className="flex h-full min-h-48 flex-col items-center justify-center px-6 text-center text-muted-foreground"><MessageSquare className="mb-3 h-8 w-8 opacity-30"/><p className="text-sm font-medium text-foreground">Ainda não tens conversas</p><p className="mt-1 text-xs">A conversa fica disponível quando uma reserva ou evento pago é confirmado.</p></div>:filteredContacts.map(contact=><button key={contact.id} type="button" onClick={()=>selectContact(contact.id)} className={`flex min-h-20 w-full items-center gap-3 rounded-xl border p-3 text-left transition-colors ${activeConversationId===contact.id?'border-primary/20 bg-primary/10':'border-transparent hover:bg-muted/60'} ${contact.archived?'opacity-70':''}`}><UserAvatar name={contact.name||'Utilizador'} src={contact.avatar} size="lg" roleLabel={contact.role}/><div className="min-w-0 flex-1"><div className="mb-0.5 flex items-baseline justify-between gap-2"><h3 className="truncate text-sm font-semibold">{contact.name}</h3><span className="shrink-0 text-[11px] text-muted-foreground">{formatContactTime(contact.lastMsgDate)}</span></div><p className="truncate text-[11px] font-semibold text-primary">{contact.contextLabel||contact.role}</p>{contact.contextDetail&&<p className="truncate text-[11px] text-muted-foreground">{contact.contextDetail}</p>}<div className="mt-1 flex items-center gap-1.5"><p className={`min-w-0 flex-1 truncate text-xs ${contact.unread>0?'font-semibold text-foreground':'text-muted-foreground'}`}>{contact.lastMsg}</p>{contact.archived&&<Archive className="h-3.5 w-3.5 shrink-0 text-muted-foreground"/>}{contact.unread>0&&<span className="flex h-5 min-w-5 shrink-0 items-center justify-center rounded-full bg-primary px-1.5 text-[10px] font-bold text-primary-foreground">{contact.unread}</span>}</div></div></button>)}</div>
    </aside>
