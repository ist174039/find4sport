...
export function CreateCommunityWizard({ categories }: { categories: TaxonomyOption[] }) {
  ...
  const [formData, setFormData] = useState({ name: '', description: '', categoryId: '', privacy: 'pub', city: '', postingPolicy: 'members' as 'members' | 'reactions_only' | 'admin_only' })
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => setFormData(current => ({ ...current, [e.target.name]: e.target.value }))
  ...
  const rules = [
    ...
    { value: 'admin_only', icon: ShieldCheck, title: 'Só administradores publicam', text: 'Administradores criam publicações; os membros podem comentar e colocar gosto.' },
  ] as const

  return <div className="min-h-screen bg-background px-4 py-6 sm:py-10"><div className="mx-auto max-w-3xl">
    <div className="mb-8"><h1 className="text-center text-2xl font-bold sm:text-3xl">Criar comunidade</h1><p className="mt-2 text-center text-sm text-muted-foreground">Define identidade, modalidade, acesso e regras antes de publicar.</p><div className="mt-6 flex items-center gap-2">{[1,2,3].map(i => <div key={i} className={`h-2 flex-1 rounded-full ${step >= i ? 'bg-primary' : 'bg-muted'}`} />)}</div></div>
    <form onSubmit={handleSubmit} className="overflow-visible rounded-3xl border border-border bg-card shadow-sm">
