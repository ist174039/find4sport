...
export async function OverviewDashboard({ role, userId, displayName, entity }: Props) {
  ...
  if (role === 'athlete') {
    const [{ count: favorites }, { count: registrations }, { count: following }, { data: participations }] = await Promise.all([
    ...
    metrics = [
      ...
      { label: 'Inscrições', value: registrations || 0, icon: CalendarCheck },
  ...
  if (role === 'professional') {
    ...
    activityRows = (recentReservations || []).map((reservation: any) => ({ id: reservation.id, title: `Reserva #${reservation.id.slice(0, 6)}`, subtitle: `${new Date(reservation.date).toLocaleDateString('pt-PT')} · ${String(reservation.start_time).slice(0, 5)}`, status: reservation.status }))
  ...
  return (
    <DashboardPage>
      <DashboardPageHeader
        ...
        description={role === 'athlete' ? 'Acompanha a tua atividade na Find4Sport.' : 'Acompanha operação, clientes e desempenho num único lugar.'}
        action={<Button asChild><Link href={primaryHref}>{primaryLabel}<ArrowRight className="ml-2 h-4 w-4" /></Link></Button>}
      />

      <DashboardStatGrid>
        {metrics.map((metric) => <DashboardStat key={metric.label} label={metric.label} value={metric.value} hint={metric.hint} icon={<metric.icon className="h-5 w-5" />} />)}
      </DashboardStatGrid>
      ...
      {role !== 'athlete' && (
        <DashboardSection>
          <div className="flex items-center gap-4">
            ...
            <div><p className="font-semibold">Reputação</p><p className="text-sm text-muted-foreground">Rating médio: {entity?.rating_avg ? Number(entity.rating_avg).toFixed(1) : 'Sem avaliações'} · {entity?.review_count || 0} avaliações</p></div>
          </div>
        </DashboardSection>
      )}

      <div className="grid gap-5 lg:grid-cols-3">
        <DashboardSection title={activityTitle} description="Dados reais registados na plataforma." className="lg:col-span-2">
          {activityRows.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">Ainda não existem registos para apresentar.</p> : <div className="space-y-3">{activityRows.map((row) => <div key={row.id} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-muted/20 p-3"><div className="min-w-0"><p className="truncate text-sm font-medium">{row.title}</p><p className="text-xs text-muted-foreground">{row.subtitle}</p></div>{row.status && <Badge variant="outline" className="shrink-0">{row.status}</Badge>}</div>)}</div>}
        </DashboardSection>

        <DashboardSection title="Mensagens recentes" description="Últimas mensagens recebidas.">
          {(recentMessages || []).length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">Sem mensagens recebidas.</p> : <div className="space-y-3">{(recentMessages || []).map((message) => { const sender = senderMap.get(message.sender_id) as any; return <div key={message.id} className="rounded-xl border border-border p-3"><div className="mb-1 flex items-center justify-between gap-2"><p className="truncate text-sm font-semibold">{sender?.full_name || 'Utilizador'}</p>{!message.read_at && <Badge>Novo</Badge>}</div><p className="line-clamp-2 text-xs text-muted-foreground">{message.content}</p></div> })}<Button asChild variant="outline" className="min-h-11 w-full"><Link href="/dashboard/mensagens">Abrir mensagens</Link></Button></div>}
        </DashboardSection>
      </div>
    </DashboardPage>
  )
