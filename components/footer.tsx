...
function FooterColumn({ title, links }: { title: string; links: Array<{ name: string; href: string }> }) {
  ...
  return <div><h2 className="text-sm font-semibold text-foreground">{title}</h2><ul className="mt-3 space-y-1 sm:mt-4 sm:space-y-2">{links.map(link => <li key={link.href}><Link href={link.href} className="inline-flex min-h-10 items-center text-sm text-muted-foreground transition-colors hover:text-primary">{link.name}</Link></li>)}</ul></div>
}

export function Footer() {
  const pathname = usePathname()
  const utilityScreen = pathname === '/feed' || pathname.startsWith('/pesquisa')
  if (utilityScreen) return null
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-14">
        <div className="grid gap-8 lg:grid-cols-[1.2fr_3fr]">
          <div>
            ...
            <p className="mt-3 max-w-sm text-sm leading-relaxed text-muted-foreground">Profissionais, espaços, eventos e comunidades desportivas numa única plataforma.</p>
          </div>
        ...
        <div className="mt-8 border-t border-border pt-5 text-center text-xs text-muted-foreground sm:mt-10 sm:text-sm">© {new Date().getFullYear()} FIND4SPORT. Todos os direitos reservados.</div>
      </div>
    </footer>
  )
}
