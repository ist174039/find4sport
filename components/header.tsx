...
import { publicPrimaryNavigation, publicSecondaryNavigation } from '@/lib/navigation/public'
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-1 md:flex" aria-label="Navegação principal">
          {publicPrimaryNavigation.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={cn('flex min-h-10 items-center gap-2 rounded-lg px-3 py-2 text-sm font-medium transition-colors', active ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-accent hover:text-accent-foreground')}>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              {publicSecondaryNavigation.map((item) => (
                <DropdownMenuItem key={item.href} render={<Link href={item.href} className="flex cursor-pointer items-center"><item.icon className="mr-2 h-4 w-4" />{item.name}</Link>} />
              ))}
            </DropdownMenuContent>
          </DropdownMenu>
        </nav>
        ...
        <div className="flex items-center gap-1 sm:gap-2">
          ...
          <Button variant="ghost" size="icon" asChild className="hidden sm:flex">
            <Link href="/pesquisa" aria-label="Pesquisar"><Search className="h-5 w-5" /></Link>
          </Button>

          {user ? (
            <>
              <Button variant="ghost" size="icon" asChild className="relative h-11 w-11">
                <Link href="/dashboard/notificacoes" aria-label="Notificações">
                  <Bell className="h-5 w-5" />
                  {notificationCount > 0 && <Badge variant="destructive" className="absolute right-0.5 top-0.5 h-5 min-w-5 rounded-full px-1 text-[10px]">{notificationCount > 9 ? '9+' : notificationCount}</Badge>}
                </Link>
              </Button>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger className="relative hidden h-10 w-10 items-center justify-center rounded-full transition-opacity hover:opacity-90 md:flex">
                  <Avatar className="h-9 w-9">
                    ...
                    <AvatarFallback className="bg-primary font-bold text-primary-foreground">{user.full_name?.charAt(0) || user.email.charAt(0).toUpperCase()}</AvatarFallback>
                  </Avatar>
                </DropdownMenuTrigger>
            ...
            <>
              <Button variant="ghost" asChild className="hidden sm:flex"><Link href="/auth/login"><LogIn className="mr-2 h-4 w-4" />Entrar</Link></Button>
              <Button asChild size="sm" className="hidden sm:inline-flex"><Link href="/auth/registar">Registar</Link></Button>
            </>
          )}
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            ...
            <SheetContent side="right" className="w-[min(86vw,320px)] p-0">
              ...
              <div className="flex flex-col gap-1 p-3">
                ...
                {publicSecondaryNavigation.map((item) => (
                  <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)} className="flex min-h-11 items-center gap-3 rounded-xl px-3 text-sm text-muted-foreground hover:bg-muted hover:text-foreground">
                ...
                {user ? (
                  <>
                    <div className="mb-2 flex items-center gap-3 rounded-xl bg-muted/40 p-3">
                      <Avatar className="h-10 w-10"><AvatarImage src={user.avatar_url || undefined} /><AvatarFallback>{user.full_name?.charAt(0) || 'U'}</AvatarFallback></Avatar>
                      <div className="min-w-0"><p className="truncate text-sm font-semibold">{user.full_name || 'Utilizador'}</p><p className="truncate text-xs text-muted-foreground">{user.email}</p></div>
                    </div>
                  ...
                  <div className="grid gap-2">
                    <Button asChild variant="outline"><Link href="/auth/login" onClick={() => setMobileMenuOpen(false)}>Entrar</Link></Button>
                    <Button asChild><Link href="/auth/registar" onClick={() => setMobileMenuOpen(false)}>Criar conta</Link></Button>
                  </div>
                )}
