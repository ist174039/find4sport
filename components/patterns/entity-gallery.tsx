  ...
  return <>
    <div className="space-y-3">
      <button type="button" onClick={() => setOpen(true)} className="group relative block aspect-[4/3] w-full overflow-hidden rounded-2xl bg-muted sm:aspect-[16/9]" aria-label="Abrir galeria">
        <AppImage src={clean[selected]} alt={`${alt} — fotografia ${selected + 1}`} fill sizes="(min-width: 1024px) 768px, 100vw" className="object-cover transition duration-300 group-hover:scale-[1.02]" />
      ...
      {clean.length > 1 && <div className="grid grid-cols-4 gap-2">{clean.slice(0, 4).map((url, index) => <button key={`${url}-${index}`} type="button" onClick={() => setSelected(index)} className={`relative aspect-square overflow-hidden rounded-xl border-2 bg-muted transition ${selected === index ? 'border-primary' : 'border-transparent opacity-80 hover:opacity-100'}`}><AppImage src={url} alt="" fill sizes="160px" className="object-cover" /></button>)}</div>}
    </div>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="h-[100dvh] max-h-[100dvh] w-screen max-w-none rounded-none border-0 bg-black p-0 text-white sm:h-[90vh] sm:max-h-[90vh] sm:w-[92vw] sm:max-w-6xl sm:rounded-2xl">
        ...
        <div className="relative flex h-full items-center justify-center overflow-hidden">
          <AppImage src={clean[selected]} alt={`${alt} — fotografia ${selected + 1}`} fill sizes="100vw" className="object-contain" />
