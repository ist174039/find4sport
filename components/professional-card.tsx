...
export function ProfessionalCard({ professional, variant = 'default', showFavoriteButton = false, isFavorited = false, onFavoriteToggle, className }: ProfessionalCardProps) {
  ...
  return <Card className={cn('group h-full overflow-hidden rounded-2xl border-border/70 transition duration-200 hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-md', className)}><Link href={`/profissionais/${professional.public_slug || professional.id}`} className="block h-full"><div className="relative aspect-[4/3] overflow-hidden bg-muted sm:aspect-[16/10]">{professional.gallery_urls?.[0]?<Image src={professional.gallery_urls[0]} alt={displayName} fill sizes="(min-width:1280px) 25vw, (min-width:768px) 33vw, 100vw" className="object-cover transition-transform duration-300 group-hover:scale-[1.03]"/>:<div className="flex h-full items-center justify-center"><Avatar className="h-20 w-20 sm:h-24 sm:w-24"><AvatarImage src={professional.avatar_url || undefined} alt={displayName}/><AvatarFallback className="bg-primary text-xl text-primary-foreground">{initials}</AvatarFallback></Avatar></div>}{professional.is_premium&&<Badge className="absolute left-2 top-2 bg-amber-500 px-2 py-0.5 text-[10px]">Premium</Badge>}{distance&&<span className="absolute bottom-2 right-2 rounded-full bg-background/95 px-2 py-1 text-[10px] font-semibold shadow-sm"><Navigation className="mr-1 inline h-3 w-3 text-primary"/>{distance}</span>}{showFavoriteButton&&<Button variant="secondary" size="icon" className="absolute right-2 top-2 h-9 w-9 rounded-full" onClick={e=>{e.preventDefault();onFavoriteToggle?.(professional.id)}}><Heart className={cn('h-4 w-4',isFavorited&&'fill-destructive text-destructive')}/></Button>}</div><CardContent className="p-3.5 sm:p-4"><div className="flex items-start justify-between gap-2"><h3 className="line-clamp-1 flex min-w-0 items-center gap-1 text-base font-semibold">{displayName}{professional.is_verified&&<BadgeCheck className="h-4 w-4 shrink-0 text-primary"/>}</h3>{professional.rating_avg!=null&&<span className="flex shrink-0 items-center gap-1 text-xs font-semibold"><Star className="h-3.5 w-3.5 fill-amber-500 text-amber-500"/>{professional.rating_avg.toFixed(1)}</span>}</div>{professional.address&&<p className="mt-1.5 flex items-center gap-1 text-xs text-muted-foreground"><MapPin className="h-3.5 w-3.5 shrink-0"/><span className="truncate">{professional.address}</span></p>}<div className="mt-3 flex min-h-6 flex-wrap items-center justify-between gap-2">{professional.categories?.[0]?<Badge variant="secondary" className="max-w-[65%] truncate text-[10px]">{professional.categories[0].emoji} {professional.categories[0].name}</Badge>:<span/>}{price&&<span className="shrink-0 text-xs font-bold text-primary">Média {price}</span>}</div><p className="mt-1.5 text-[11px] text-muted-foreground">{professional.review_count ?? 0} avaliações</p></CardContent></Link></Card>
}
...
export function ProfessionalGrid({ professionals, variant='default', columns=3, showFavoriteButton=false, favoritedIds=[], onFavoriteToggle, className }: ProfessionalGridProps) {
  const gridCols={2:'grid-cols-1 sm:grid-cols-2',3:'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3',4:'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}
  ...
  return <div className={cn('grid gap-4 lg:gap-5',gridCols[columns],className)}>{professionals.map(pro=><ProfessionalCard key={pro.id} professional={pro} variant={variant} showFavoriteButton={showFavoriteButton} isFavorited={favoritedIds.includes(pro.id)} onFavoriteToggle={onFavoriteToggle}/>)}</div>
}
