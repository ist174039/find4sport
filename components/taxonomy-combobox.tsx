...
export function TaxonomyCombobox({
  ...
  const choose = (id: string) => {
    if (!multiple) { onChange(id); closeDropdown(); return }
  ...
  return <div ref={rootRef} className={`relative ${className}`}>
    <button type="button" disabled={disabled} onClick={() => { if (open) closeDropdown(); else setOpen(true) }} aria-haspopup="listbox" aria-expanded={open} className="flex min-h-11 w-full items-center justify-between gap-3 rounded-xl border border-input bg-background px-3 py-2 text-left text-sm outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/15 disabled:cursor-not-allowed disabled:opacity-50">
    ...
    {open && <div className="absolute z-50 mt-2 w-full min-w-[280px] overflow-hidden rounded-2xl border border-border bg-popover shadow-xl">
      <div className="border-b p-2"><label className="relative block"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input ref={searchRef} value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => { if (event.key === 'Escape') closeDropdown() }} placeholder={searchPlaceholder} className="h-11 w-full rounded-xl border border-input bg-background pl-9 pr-9 text-base outline-none focus:border-primary" />{query && <button type="button" onClick={() => setQuery('')} className="absolute right-1 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted" aria-label="Limpar pesquisa"><X className="h-4 w-4" /></button>}</label></div>
      <div role="listbox" aria-multiselectable={multiple || undefined} className="max-h-[min(52vh,360px)] overflow-y-auto p-1.5">
        {rows.length === 0 ? <p className="px-3 py-8 text-center text-sm text-muted-foreground">{emptyLabel}</p> : rows.map(({ option, parent, depth }) => {
          ...
          return <button key={option.id} type="button" role="option" aria-selected={selected} disabled={blocked} onClick={() => choose(option.id)} className={`flex min-h-11 w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm transition ${selected ? 'bg-primary/10 text-primary' : 'hover:bg-muted'} ${blocked ? 'opacity-40' : ''}`}>
            <span className={`flex min-w-0 flex-1 items-center gap-2 ${depth ? 'pl-5' : 'font-semibold'}`}>{depth > 0 && <span className="text-muted-foreground">↳</span>}<span className="min-w-0"><span className="block truncate">{option.name}</span>{depth > 0 && parent && <span className="block truncate text-[11px] font-normal text-muted-foreground">{parent.name}</span>}</span></span>{selected && <Check className="h-4 w-4 shrink-0" />}
          </button>
        })}
      </div>
