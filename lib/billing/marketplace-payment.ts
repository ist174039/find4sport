...
export function marketplacePaymentIntentData(quote:MarketplacePaymentQuote,metadata:Record<string,string>){const fullMetadata={...metadata,provider_user_id:quote.providerUserId,connected_account_id:quote.stripeAccountId,plan_code:quote.planCode,base_amount_cents:String(quote.baseAmountCents),customer_fee_cents:String(quote.customerFeeCents),platform_commission_cents:String(quote.commissionCents),application_fee_cents:String(quote.applicationFeeCents),commission_rate:String(quote.commissionRate),customer_fee_rate:String(quote.customerFeeRate)};const deferred=['service_reservation','space_reservation'].includes(metadata.transaction_type);return deferred?{metadata:fullMetadata,transfer_group:`f4s:${metadata.transaction_type}:${metadata.reservation_id||'pending'}`}:{application_fee_amount:quote.applicationFeeCents,transfer_data:{destination:quote.stripeAccountId},metadata:fullMetadata}}
export function marketplaceLineItems(quote:MarketplacePaymentQuote,name:string,description?:string){const items=[{price_data:{currency:'eur',product_data:{name,...(description?{description}:{})},unit_amount:quote.baseAmountCents},quantity:1}];if(quote.customerFeeCents>0)items.push({price_data:{currency:'eur',product_data:{name:'Taxa de serviço Find4Sport',description:'Taxa de serviço apresentada antes da confirmação do pagamento.'},unit_amount:quote.customerFeeCents},quantity:1});return items}
